...
applyTo:
  - '**'
description: |
  MCP Memory Query Guide for ICTServe project.
  AI agent memory management using MCP Memory Server.
  Query patterns, entity architecture, and lifecycle protocols.
tags:
  - mcp
  - memory
  - knowledge-graph
...
**Framework**: MCP Protocol v2025-06-18  
**Architecture**: Knowledge Graph (entities, relations, observations)  
**Purpose**: AI agent memory management and context preservation
...
When using MCP Memory, ensure:

- [ ] Query existing memory BEFORE creating new entities
- [ ] Use `search_nodes()` to find relevant knowledge
- [ ] Use `open_nodes()` for specific entity retrieval
- [ ] Follow relations to discover connected knowledge
- [ ] Create entities with descriptive observations (10-15 per entity)
- [ ] Link entities via relations (documents, implements, uses, related_to)
- [ ] Update entities with `add_observations()` for new facts
- [ ] Use specific entity names (e.g., `Livewire_3_Component_Patterns`)
- [ ] Include date information when time-sensitive
- [ ] Test queries after creating to verify discoverability

---

**Status**: ✅ Active for ICTServe MCP Memory management  
**Version**: 1.0.0  
