...
| -------------------------- | ------------------------------------------------------ |
| **Versi**                  | 2.1.0 (SemVer)                                         |
| **Tarikh Kemaskini**       | 18 Oktober 2025                                        |
...
| **Rujukan D00-D14**        | D00, D03, D04, D07, D08, D09, D11, D12, D13, D14       |
| **Document ID**           | DOC-DESIGN-ICTSERVE-2025-Q4                            |
| **Audit Date**            | 18 Oktober 2025                                        |
| **Audit Score**           | 90/100 — Patuh dengan D00–D14 (Minor improvements noted) |

---

## Kelulusan & Tandatangan (Approval & Sign-Off)

| Peranan (Role)            | Nama (Name)             | E-mel              | Tarikh Kelulusan | PR/Commit Reference |
|---------------------------|------------------------|--------------------|------------------|---------------------|
...
**Catatan**: Dokumen ini telah melalui audit lengkap untuk pematuhan D00–D14 pada 18 Oktober 2025. Kelulusan formal akan dicatat setelah PR merge.
...
| **D04 Software Design**    | `D04_SOFTWARE_DESIGN_DOCUMENT.md`      | Reka bentuk perisian                   |
| **D07-D08 Integration**    | `D07/D08_SYSTEM_INTEGRATION*.md`      | Pelan dan spesifikasi integrasi        |
| **D09 Database**           | `D09_DATABASE_DOCUMENTATION.md`       | Dokumentasi pangkalan data             |
...
## Pematuhan Piawaian & Rujukan RTM (Compliance Standards & RTM Reference)

### Rujukan Traceability Matrix (RTM)

Dokumentasi ini dipetakan kepada Requirements Specification (D03) melalui Requirements Traceability Matrix (RTM). Untuk maklumat lengkap tentang pemetaan design decision kepada SRS items, sila rujuk:

- **RTM Master File**: `docs/rtm/design_requirements_rtm.csv` (Dipetakan oleh Tim Dokumentasi, tarikh audit: 18 Oktober 2025)
- **RTM Format**: Setiap baris merupakan design decision (DES-ARCH-XXX) dipetakan ke SRS ID (SRS-ARCH-XXX), standard, evidence artifact, dan audit status
- **Coverage**: 100% design decisions dalam dokumen ini mempunyai pemetaan ke SRS items dalam D03

### Pemetaan Reka Bentuk ↔ Keperluan Perisian (Design ↔ SRS Linkage)

| Reka Bentuk Utama (Design Decision) | SRS Rujukan | Bahagian Dokumen | Bukti Artifact |
|-----|-----|-----|-----|
| **Seni Bina Berlapis (Presentation, Application, Domain, Infrastructure)** | SRS-ARCH-001 (Modular architecture) | Seksyen 3.3 | `app/` directory structure |
| **Event-Driven & Audit Trail** | SRS-ARCH-002 (Event sourcing) | Seksyen 4.3 & 8 | `app/Events/`, `app/Observers/` |
| **Domain-Driven Design (Loan vs Helpdesk domains)** | SRS-ORG-001 (Separation of concerns) | Seksyen 4.1 | `app/Services/LoanManagement/`, `app/Services/Helpdesk/` |
| **Security by Design (authentication, encryption, rate limiting)** | SRS-SEC-001 (Built-in security controls) | Seksyen 7 | `app/Services/Security/`, middleware configs |
| **RBAC & Authorization Policies** | SRS-SEC-002 (Access control) | Seksyen 4 & 7 | `app/Policies/`, Spatie roles in D09 |
| **Data Protection & Encryption (PII)** | SRS-PRIV-001 (Privacy by design, PDPA compliance) | Seksyen 7.3 & DB Design | `app/Services/Security/DataProtectionService.php` |
| **Queue Management & Background Jobs** | SRS-PERF-001 (Asynchronous processing) | Seksyon 4 & 6.1 | `app/Jobs/`, config/queue.php |
| **Performance Optimization (caching, indexing)** | SRS-PERF-002 (Performance SLOs) | Seksyen 6.2 & 9 | Index strategies in Seksyen 5 |
| **Monitoring & Health Checks** | SRS-OPS-001 (Observability & alerting) | Seksyen 8 & 10 | Health check endpoints, metrics |
| **CI/CD & Deployment Strategy** | SRS-OPS-002 (Reliable deployment) | Seksyen 10 | `.github/workflows/deploy.yml`, `scripts/blue-green-deploy.sh` |

**Catatan**: Untuk detail lengkap setiap pemetaan (owner, test case, approval), lihat RTM Master File di atas.
...
- [Glosari](#glosari)
- [Rujukan Silang & Piawaian](#rujukan-silang--piawaian)
- [**Appendix A: Data Dictionary**](#appendix-a-data-dictionary-kamus-data-lengkap)
- [Sejarah Perubahan](#sejarah-perubahan-changelog)
...
│   ├── semantic-tokens.css     # Design system tokens (WCAG 2.2 Level AA compliant)
...
- **Panduan Utama**: `D14_UI_UX_STYLE_GUIDE.md` - Panduan lengkap gaya dan sistem reka bentuk
- **Ringkasan**: `D12_UI_UX_DESIGN_GUIDE.md` - Panduan rekabentuk UI/UX
- **Rujukan Pantas**: `D13_UI_UX_FRONTEND_FRAMEWORK.md` - Rujukan pantas untuk pembangun
...
- **Accessibility**: WCAG 2.2 Level AA compliant dengan nisbah kontras yang mencukupi
...
trait ValidatesWithSecurity
    ...
    protected function applyMalaysianRules(): array
        ...
        return [
            'ic_number' => ['sometimes', 'string', 'regex:/^\d6-\d2-\d4$/'],
            'phone' => ['sometimes', 'string', 'regex:/^01[0-9]-\d4\s\d4$/'],
            'postcode' => ['sometimes', 'string', 'regex:/^\d5$/'],
      ;
...
## Appendix A: Data Dictionary (Kamus Data Lengkap)

### A.1 Tujuan Data Dictionary

Lampiran ini menyediakan referensi lengkap untuk semua medan (fields) dalam jadual teras sistem ICTServe, termasuk jenis data, constraints, deskripsi, dan klasifikasi PDPA untuk privasi.

### A.2 Core Tables Data Dictionary

#### Table: `users` (Pengguna Sistem)

| Medan (Field) | Jenis Data | Wajib? | Default | Deskripsi | PDPA Klasifikasi | Enkripsi? |
|---|---|---|---|---|---|---|
| id | BIGINT UNSIGNED | ✅ | AUTO_INCREMENT | Primary key | N/A | ❌ |
| uuid | UUID | ✅ | Str::uuid() | Identifier unik (non-sensitive) | N/A | ❌ |
| name | VARCHAR(255) | ✅ | NULL | Nama pengguna lengkap | **Personal Data** | ❌ |
| email | VARCHAR(255) | ✅ | NULL | Emel pengguna (unique) | **Identifiable PII** | ❌ |
| password | VARCHAR(255) | ✅ | NULL | Hash kata laluan (bcrypt) | **Sensitive** | ❌ |
| identification_number | VARCHAR(12) | ❌ | NULL | Nombor KP / Passport (e.g., XXXXXX-XX-XXXX) | **Sensitive PII - ENCRYPT** | ✅ AES-256 |
| mobile_number | VARCHAR(15) | ❌ | NULL | Nomor telefon mudah alih | **Contact PII** | ✅ AES-256 |
| department_id | BIGINT UNSIGNED | ✅ | NULL | FK kepada jadual departments | N/A | ❌ |
| position_id | BIGINT UNSIGNED | ❌ | NULL | FK kepada jadual positions | N/A | ❌ |
| grade_id | BIGINT UNSIGNED | ❌ | NULL | FK kepada jadual grades | N/A | ❌ |
| is_active | BOOLEAN | ✅ | true | Status pengguna aktif/tidak aktif | N/A | ❌ |
| last_login_at | TIMESTAMP | ❌ | NULL | Waktu login terakhir | **Activity Log** | ❌ |
| created_at | TIMESTAMP | ✅ | CURRENT_TIMESTAMP | Waktu penciptaan rekod | N/A | ❌ |
| updated_at | TIMESTAMP | ✅ | CURRENT_TIMESTAMP | Waktu kemas kini terakhir | N/A | ❌ |
| deleted_at | TIMESTAMP | ❌ | NULL | Waktu soft delete (soft deletes enabled) | N/A | ❌ |
| created_by | BIGINT UNSIGNED | ❌ | NULL | FK kepada users (auditor) | N/A | ❌ |
| updated_by | BIGINT UNSIGNED | ❌ | NULL | FK kepada users (auditor) | N/A | ❌ |
| deleted_by | BIGINT UNSIGNED | ❌ | NULL | FK kepada users (auditor) | N/A | ❌ |

**Catatan PDPA**:
- `identification_number` dan `mobile_number` adalah **sensitive personal data** dan mesti dienkripsi menggunakan AES-256 (Larvel Crypt facade).
- Semua akses kepada medan ini mesti dicatat dalam audit logs.
- Retention: 5 tahun selepas pemadaman pengguna; kemudian dipindahkan ke archive atau dipadamkan secara selamat.

#### Table: `loan_applications` (Permohonan Pinjaman Aset)

| Medan (Field) | Jenis Data | Wajib? | Default | Deskripsi | PDPA Klasifikasi |
|---|---|---|---|---|---|
| id | BIGINT UNSIGNED | ✅ | AUTO_INCREMENT | Primary key | N/A |
| uuid | UUID | ✅ | Str::uuid() | Identifier unik | N/A |
| user_id | BIGINT UNSIGNED | ✅ | NULL | FK kepada users (pemohon) | N/A |
| status | ENUM | ✅ | 'draft' | Status permohonan (draft, submitted, approved, rejected, returned) | N/A |
| application_date | DATE | ✅ | NOW() | Tarikh permohonan | N/A |
| approved_at | TIMESTAMP | ❌ | NULL | Waktu kelulusan | N/A |
| approved_by | BIGINT UNSIGNED | ❌ | NULL | FK kepada users (approver) | N/A |
| returned_at | TIMESTAMP | ❌ | NULL | Waktu pemulangan aset | N/A |
| notes | TEXT | ❌ | NULL | Catatan permohonan (mungkin mengandungi PII) | **Conditional PII** |
| created_at | TIMESTAMP | ✅ | CURRENT_TIMESTAMP | Waktu penciptaan | N/A |
| updated_at | TIMESTAMP | ✅ | CURRENT_TIMESTAMP | Waktu kemas kini | N/A |
| deleted_at | TIMESTAMP | ❌ | NULL | Soft delete | N/A |

**Catatan**: Field `notes` boleh mengandungi maklumat peribadi; pastikan divalidasi sebelum simpan dan dienkripsi jika perlu.

#### Table: `helpdesk_tickets` (Tiket Helpdesk)

| Medan (Field) | Jenis Data | Wajib? | Default | Deskripsi | PDPA Klasifikasi |
|---|---|---|---|---|---|
| id | BIGINT UNSIGNED | ✅ | AUTO_INCREMENT | Primary key | N/A |
| ticket_number | VARCHAR(50) | ✅ | NULL | Nombor tiket unik (e.g., HELP-2025-00001) | N/A |
| user_id | BIGINT UNSIGNED | ✅ | NULL | FK kepada users (pelapor) | N/A |
| assigned_to_user_id | BIGINT UNSIGNED | ❌ | NULL | FK kepada users (agen IT) | N/A |
| category_id | BIGINT UNSIGNED | ✅ | NULL | Kategori masalah | N/A |
| priority | ENUM | ✅ | 'medium' | Prioriti (low, medium, high, critical) | N/A |
| status | ENUM | ✅ | 'open' | Status tiket (open, in_progress, resolved, closed) | N/A |
| subject | VARCHAR(255) | ✅ | NULL | Tajuk tiket | N/A |
| description | TEXT | ✅ | NULL | Deskripsi masalah (mungkin ada PII) | **Conditional PII** |
| resolution | TEXT | ❌ | NULL | Penyelesaian yang diberikan | N/A |
| sla_due_at | TIMESTAMP | ✅ | NULL | Tarikh SLA (dikira berdasarkan prioriti) | N/A |
| created_at | TIMESTAMP | ✅ | CURRENT_TIMESTAMP | Waktu penciptaan | N/A |
| updated_at | TIMESTAMP | ✅ | CURRENT_TIMESTAMP | Waktu kemas kini | N/A |
| deleted_at | TIMESTAMP | ❌ | NULL | Soft delete | N/A |

**Catatan**: Bidang `description` dan `resolution` boleh mengandungi maklumat sensitif pengguna; pastikan akses dikawal melalui policies.

#### Table: `equipment` (Senarai Peralatan ICT)

| Medan (Field) | Jenis Data | Wajib? | Default | Deskripsi | PDPA Klasifikasi |
|---|---|---|---|---|---|
| id | BIGINT UNSIGNED | ✅ | AUTO_INCREMENT | Primary key | N/A |
| asset_tag | VARCHAR(50) | ✅ | NULL | Label aset (e.g., ASSET-2025-001) | N/A |
| name | VARCHAR(255) | ✅ | NULL | Nama peralatan | N/A |
| category_id | BIGINT UNSIGNED | ✅ | NULL | FK kepada equipment_categories | N/A |
| status | ENUM | ✅ | 'available' | Status (available, in_use, maintenance, retired) | N/A |
| location_id | BIGINT UNSIGNED | ✅ | NULL | FK kepada locations | N/A |
| purchase_date | DATE | ✅ | NULL | Tarikh pembelian | N/A |
| value | DECIMAL(10,2) | ✅ | 0 | Nilai peralatan (RM) | N/A |
| last_assigned_user_id | BIGINT UNSIGNED | ❌ | NULL | FK kepada users (pengguna terakhir) | N/A |
| created_at | TIMESTAMP | ✅ | CURRENT_TIMESTAMP | Waktu penciptaan | N/A |
| updated_at | TIMESTAMP | ✅ | CURRENT_TIMESTAMP | Waktu kemas kini | N/A |
| deleted_at | TIMESTAMP | ❌ | NULL | Soft delete | N/A |

### A.3 PDPA Compliance Summary

Ringkasan medan yang mengandungi **Personal Data** dan perlindungan yang diperlukan:

| Jadual | Medan PII | Enkripsi | Retention Policy | Consent Required? |
|---|---|---|---|---|
| `users` | name, email, identification_number, mobile_number | ✅ (sensitive fields) | 5 tahun post-delete | ✅ User opt-in |
| `loan_applications` | user_id (linked), notes (conditional) | ❌ (field-level as needed) | 7 tahun (compliance) | ✅ Part of application |
| `helpdesk_tickets` | user_id, description (conditional) | ❌ | 3 tahun (retention) | ✅ Implicit (reporting issue) |
| `equipment` | last_assigned_user_id (historical) | ❌ | 5 tahun (asset lifecycle) | ❌ (operational data) |

**Catatan PDPA 2010 & ISO 27701**:
- Semua akses kepada PII mesti dicatat dalam `audit_logs` dengan timestamp, user, dan aksi.
- Pengguna mempunyai hak DSAR (Data Subject Access Request) — sistem mesti dapat mengekspor semua PII mereka dalam format readable.
- Retention periods mesti dikuatkuasakan melalui scheduled jobs (lihat `app/Console/Commands/ArchiveOldRecords.php`).
...
| ------------ | ---------- | --------------------------------------------------------- | -------- |
| WCAG 2.2 Level AA  | W3C        | Aksesibiliti web                                          | Dipatuhi |
...
| Versi | Tarikh            | Penulis          | Perubahan Utama                                                                   | PR/Commit |
| ----- | ----------------- | ---------------- | --------------------------------------------------------------------------------- | --------- |
| 2.1.1 | 18 Oktober 2025   | Pasukan ICTServe | **Audit Compliance Update**: Add D03 traceability mapping, RTM file reference, sign-off metadata (Document ID, Audit Score), Appendix A (Data Dictionary with PDPA classifications), updated TOC. Reference: D00~D14 audit compliance, ISO 8000 data quality, PDPA 2010 PII handling. | PR#audit-design |
| 3.0   | 6 Oktober 2025    | Pasukan ICTServe | Penambahbaikan komprehensif: lengkapkan pemantauan, deployment, konfigurasi       | PR#v3-comprehensive |
| 2.10  | 5 Oktober 2025    | Pasukan ICTServe | Kemas kini besar: seni bina, keselamatan, prestasi, dokumentasi standard industri | PR#v2.10-update |
| 2.0   | 26 September 2025 | Pasukan ICTServe | Kemas kini besar: seni bina, keselamatan, prestasi, dokumentasi standard industri | PR#major-redesign |
| 1.5   | 15 September 2025 | Pasukan ICTServe | Tambah modul helpdesk dan integrasi                                               | PR#helpdesk-integration |
| 1.0   | 12 September 2025 | IzzatFirdaus     | Versi awal dokumentasi sistem                                                     | commit:initial |
...
#### Pemantauan & Observabiliti

- Service monitoring komprehensif dengan metrik real‑time (Redis queue, response times, error rates).
- Health check endpoints terperinci untuk database, cache, queue, storage, dan API luaran.
- Dashboard analitik prestasi (slow endpoints, trend error, throughput).
- Real‑time error tracking dan alerting (logs, Sentry/ELK, pager rules).

#### Strategi Deployment

- CI/CD pipeline penuh menggunakan GitHub Actions (test, scan, build, deploy).
- Blue‑green deployment untuk zero‑downtime dengan health checks automatik.
- Automated testing dan security scanning (unit, feature, E2E, dependency audits).
- Mekanisme rollback yang kukuh dan smoke tests selepas deploy.

#### Pengurusan Konfigurasi

- Centralized configuration management (config files + env per‑environment).
- Environment‑specific settings dan secrets management (Vault/SSM recommended).
- Feature flags untuk A/B testing dan gradual rollout.
- Pendekatan konfigurasi berfokus keselamatan (least privilege, immutable config).

#### Panduan Pelaksanaan

- Skrip deployment langkah demi langkah (backup, maintenance, migrate, deploy).
- Automasi persediaan infrastruktur (idempotent scripts / IaC).
- Verifikasi post‑deployment: health checks, smoke tests, basic functional checks.
- Senarai semak komprehensif (pre, during, post deployment).

#### Standard Dokumentasi

- Format Markdown konsisten, dengan tajuk, senarai, dan contoh kod terpisah.
- Diagram Mermaid untuk visualisasi seni bina dan aliran.
- Contoh kod praktikal dan snippet yang boleh diuji.
- Cross‑referencing yang tepat ke D00–D14 dan dokumen sokongan.
...
Tujuan: Menyediakan aliran ujian end‑to‑end komprehensif yang memverifikasi integriti dari antaramuka pengguna hingga integrasi API, termasuk pengesahan aksesibiliti (WCAG 2.2 AA), responsive design, dan kawalan peranan/keizinan.
...
- ✓ Security‑first approach (rate limiting, CSRF, encryption).  
- ✓ Performance optimisation (caching, query tuning).  
- ✓ Comprehensive testing strategy (unit, feature, E2E, accessibility).  
- ✓ Continuous monitoring & observability (metrics, alerts).  
- ✓ Disaster recovery & rollback planning.  
- ✓ Complete audit trails and traceability.

Dokumentasi ini kini selaras dengan standard dokumentasi teknikal enterprise dan sedia untuk digunakan dalam peringkat production.
