...
class TestCountPreservationPropertyTest extends TestCase
{
    ...
    public function method_count_is_preserved(string $filePath): void
    {
    ...
    #[Test]
    #[DataProvider('provideTestFiles')]
    public function method_signatures_are_consistent(string $filePath): void
    {
        $content = file_get_contents($filePath);
        $this->assertNotFalse($content, "Could not read file: {$filePath}");
        ...
        $allTestMethods = [...$allTestMethods, ...$legacyMatches[1]];

        // Modern test methods with #[Test] attribute
        preg_match_all('/#\[Test\].*?public function (\w+)\([^)]*\):\s*void/ms', $content, $modernMatches);
        $allTestMethods = [...$allTestMethods, ...$modernMatches[1]];

        // Always make at least one assertion
        $this->assertIsArray($allTestMethods, 'Test methods should be an array');
    ...
    public function data_provider_methods_are_preserved(string $filePath): void
    {
        ...
        // Find actual data provider methods (both static and non-static)
        preg_match_all('/public\s+(?:static\s+)?function\s+(\w+Provider|\w+DataProvider|\w+Data|provide\w+)\(\):\s*array/', $content, $providerMethods);
        $actualProviders = $providerMethods[1];

        // Also check for methods ending with 'Provider' or containing 'data' or starting with 'provide'
        preg_match_all('/public\s+(?:static\s+)?function\s+(\w*[Pp]rovider\w*|\w*[Dd]ata\w*|provide\w+)\(\):\s*array/', $content, $additionalProviders);
        $actualProviders = [...$actualProviders, ...$additionalProviders[1]];
        $actualProviders = array_unique($actualProviders);

        // Always make at least one assertion
        $this->assertIsArray($referencedProviders, 'Referenced providers should be an array');
        ...
        foreach ($actualProviders as $providerMethod) {
            $this->assertTrue(
                preg_match("/function\s+{$providerMethod}\(\):\s*array/", $content) === 1,
     ...
     * with extends TestCase (or DuskTestCase for Browser tests) and appropriate use statements.
     */
    #[Test]
    #[DataProvider('provideTestFiles')]
    public function class_structure_is_preserved(string $filePath): void
    {
        $content = file_get_contents($filePath);
        $this->assertNotFalse($content, "Could not read file: {$filePath}");
    ...
    #[Test]
    #[DataProvider('provideTestFiles')]
    public function documentation_is_consistent(string $filePath): void
    {
        $content = file_get_contents($filePath);
        $this->assertNotFalse($content, "Could not read file: {$filePath}");
        ...
        foreach ($matches as $match) {
            ...
            // If method has documentation, verify it doesn't contain legacy @test annotation
            if (str_contains($docBlock, '/**')) {
                // Should not contain @test annotation (check for PHPDoc @test pattern)
    ...
    #[Test]
    #[DataProvider('provideTestFiles')]
    public function assertions_are_preserved(string $filePath): void
    {
        $content = file_get_contents($filePath);
        $this->assertNotFalse($content, "Could not read file: {$filePath}");

        // Find all assertion calls - multiple patterns:
        // 1. PHPUnit style: $this->assert*()
        // 2. Static style: self::assert*() or static::assert*()
        ...
        if ($testMethodCount > 0) {
            ...
            // No test methods - that's valid for this check
            $this->assertTrue(true, "File {$filePath} has no test methods");
        }
    }

    /**
     * Count legacy test methods (test_ prefix or testMethodName camelCase)
     */
    private function countLegacyTestMethods(string $content): int
    {
        // Count methods with test_ prefix
        preg_match_all('/public function test_\w+\(/', $content, $prefixMatches);

        // Count methods with testMethodName camelCase (not followed by #[Test])
        preg_match_all('/public function test[A-Z]\w*\(/', $content, $camelMatches);

        return \count($prefixMatches[0]) + \count($camelMatches[0]);
    }
    ...
    private function verifyTestMethodNaming(string $content, string $filePath): void
    {
        ...
        foreach ($methodNames as $methodName) {
            ...
            if (\in_array($methodName, ['setUp', 'tearDown', 'setUpBeforeClass', 'tearDownAfterClass'], true)) {
                continue;
            }
            ...
            if (preg_match("/#\[Test\].*?public function {$methodPattern}\(/s", $content)) {
                $this->assertFalse(
                    str_starts_with($methodName, 'test_'),
    ...
    /**
     * Get list of valid PHPUnit assertion methods
     */
    private function getValidAssertionMethods(): array
    {
        return [
            'assertTrue',
            'assertFalse',
            ...
            foreach ($iterator as $file) {
                if ($file->isFile() && $file->getExtension() === 'php') {
                    ...
                    if (
                        ...
                        preg_match('/^\s*\*\s*@test\s/m', $content) === 1
                    ) {
