/**
 * Filament Components Debug Tests with Percy Visual Testing
 *
 * ENHANCED VERSION with Percy Integration (December 2025):
 * - ✅ Filament 4.3.1 admin component visual testing
 * - ✅ Percy visual snapshots for admin panel validation
 * - ✅ ICTServe v3.6.1 True Hybrid Architecture support
 * - ✅ Bahasa Melayu interface visual validation
 *
 * Run: npm run test:e2e -- tests/e2e/filament.components.debug.spec.ts
 * Run with Percy: npm run test:e2e:percy -- tests/e2e/filament.components.debug.spec.ts
 */
...
test.describe(
	"Filament Components Debug Suite",
	{ tag: ["@filament", "@debug"] },
	() => {
		test("Dashboard widgets render without console errors", async ({
			adminPage,
		}) => {
			...
			await expect(
				adminPage.getByRole("heading", { name: /asset availability calendar/i })
			).toBeVisible();
			await expect(
				adminPage.locator('[data-testid="asset-calendar-grid"]')
			).toBeVisible();

			await expect(
				adminPage.getByRole("heading", { name: /critical alerts/i })
			).toBeVisible();
			await expect(
				adminPage.locator('[data-testid="critical-alerts-widget"]')
			).toBeVisible();

			await expect(
				adminPage.getByRole("heading", { name: /asset status distribution/i })
			).toBeVisible();
			await expect(
				adminPage.getByRole("heading", { name: /loan applications trend/i })
			).toBeVisible();
		});

		for (const resource of FILAMENT_RESOURCE_ROUTES) {
			test(`${resource.name} resource loads without failures`, async ({
				adminPage,
			}) => {
				...
				await expect(
					adminPage.getByRole("heading", { name: resource.heading })
				).toBeVisible();
				await expect(adminPage.getByRole("table")).toBeVisible();
			});
		}

		test("Asset availability legend exposes all statuses", async ({
			adminPage,
		}) => {
			...
			expectedLabels.forEach((label) => {
				const match = statusMetadata.find((entry) =>
		...
		test("Critical alerts widget surfaces empty state or alert actions", async ({
			adminPage,
		}) => {
			await adminPage.goto("/admin");
			await adminPage.waitForLoadState("networkidle");
			...
			const widget = adminPage.locator(
				'[data-testid="critical-alerts-widget"]'
			);
