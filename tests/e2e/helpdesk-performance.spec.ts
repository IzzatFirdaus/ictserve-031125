...
/**
 * Helpdesk Module Performance Tests with Percy Visual Testing Integration
 *
 ...
 * Run with Percy: npm run test:e2e:percy -- tests/e2e/helpdesk-performance.spec.ts
 */

test.describe("Helpdesk Module - Performance Tests with Percy", () => {
	...
	test(
		...
		{
			tag: ["@helpdesk", "@performance", "@vitals", "@smoke", "@percy"],
		},
		async ({ authenticatedPage }) => {
			...
			await takePercySnapshot(authenticatedPage, {
				name: "Helpdesk Performance - Core Web Vitals Baseline",
	...
	test(
		"02 - Helpdesk ticket list loads within acceptable time",
		{
	...
	test(
		"03 - Ticket submission form loads quickly",
		{
	...
	test(
		"04 - Pagination performance is acceptable",
		{
	...
	test(
		"05 - Form submission response time is fast",
		{
		...
		async ({ authenticatedPage }) => {
			...
			const inputs = authenticatedPage
	...
	test(
		"06 - No excessive database queries (N+1 prevention)",
		{
	...
	test(
		"07 - Static assets are cached effectively",
		{
	...
	test(
		"09 - JavaScript bundle size is optimized",
		{
	...
	test(
		"10 - Time to Interactive (TTI) is acceptable",
		{
