...
test.describe("Staff Dashboard - Responsive Behavior", () => {
    ...
    test.describe("Mobile Viewports (320px-414px): 1 Column Layout", () => {
        ...
        test("should display recent activity in single column on mobile", async ({
            ...
            // Target only cards in the activity grid - use simpler selector without responsive classes
            // Note: Cannot use .lg:grid-cols-2 in CSS selector (invalid syntax)
            const activityCards = page.locator('.grid.grid-cols-1').filter({ has: page.locator('[class*="border-slate-800"]') }).locator('> [class*="border-slate-800"]');
