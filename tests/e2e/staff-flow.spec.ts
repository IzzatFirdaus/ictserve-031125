...
test.describe('Staff User Optimized Complete Journey', () => {
  ...
  test('Complete staff journey: Welcome to Logout (optimized single session)', {
    ...
    await staffLoginPage.fillLoginForm(STAFF_CREDENTIALS.email, STAFF_CREDENTIALS.password);
    ...
    const dashboardCards = page.locator('[class*="card"], [class*="widget"], [class*="panel"], [data-testid="dashboard-stats-grid"]');
    ...
    const userMenuButton = page.getByRole('button', { name: /user menu|menu pengguna/i }).first();
    await expect(userMenuButton).toBeVisible({ timeout: 10000 });
    await userMenuButton.click();

    const logoutLink = page.getByRole('link', { name: /log out|log keluar/i }).first();
    await expect(logoutLink).toBeVisible({ timeout: 10000 });
    await logoutLink.click();
