...
/**
 * Asset Loan Module Performance Tests - REFACTORED
 *
 ...
 * - ✅ Test tags for filtering (@loan, @performance)
 ...
 *
 * Run: npm run test:e2e -- tests/e2e/loan-module-performance.refactored.spec.ts
 * Run performance tests only: npm run test:e2e -- --grep @performance
 */

test.describe('Asset Loan Module - Performance Tests', () => {
  ...
  test('01 - Core Web Vitals meet performance targets', {
    tag: ['@loan', '@performance', '@vitals', '@smoke'],
  ...
  test('02 - Guest loan request form loads quickly', {
  ...
  test('03 - Asset availability check is performant', {
    ...
    const assetSelect = authenticatedPage.locator('select[name*="asset"], select[id*="asset"]').first();

    const hasSelect = await assetSelect.isVisible({ timeout: 3000 }).catch(() => false);
  ...
  test('04 - Loan history pagination is efficient', {
  ...
  test('05 - Loan search/filter performs efficiently', {
    ...
    // Find search input
    const searchInput = authenticatedPage.getByRole('textbox', { name: /search|cari/i }).or(
      authenticatedPage.locator('input[type="search"], input[name*="search"]').first()
    );
  ...
  test('06 - Progressive loading for asset list', {
    ...
    const loader = authenticatedPage.locator('[role="progressbar"], .skeleton, .loading').first();
  ...
  test('07 - Time to Interactive (TTI) is acceptable', {
  ...
  test('08 - JavaScript bundle size is optimized', {
  ...
  test('09 - Database queries are optimized (no N+1)', {
  ...
  test('10 - Asset images load efficiently', {
  ...
  test('11 - Form submission response time is fast', {
  ...
  test('12 - Static assets are cached effectively', {
