...
test.describe('Loan Module - Best Practices Architecture', () => {
  ...
  test('13 - Loan Application - Email Approval Workflow', {
    ...
    await authenticatedPage.goto(`/loans/approve?token=${approvalToken}&action=approve`);
    
    // Verify approval confirmation
    // Using soft assertion as this depends on backend state
    await expect.soft(authenticatedPage.getByRole('alert')).toContainText(/diluluskan|approved/i);
  });

  test('14 - Loan Extension Request', {
    ...
    if (await extendButton.isVisible({ timeout: 3000 })) {
      ...
      await authenticatedPage.fill('input[name="new_return_date"]', '2025-12-15');
      await authenticatedPage.fill('textarea[name="justification"]', 'Project delayed');
      await authenticatedPage.getByRole('button', { name: /submit|hantar/i }).click();
      
      await expect(authenticatedPage.getByRole('alert')).toContainText(/berjaya|success/i);
    }
  });

  test('15 - Asset Availability Check', {
    ...
    if (await checkButton.isVisible({ timeout: 3000 })) {
      ...
      // Wait for results
      await expect(authenticatedPage.getByTestId('available-assets')).toBeVisible();
      
      // Verify assets displayed
      const assetCount = await authenticatedPage.getByTestId('asset-item').count();
      expect(assetCount).toBeGreaterThan(0);
    }
  });

  test('16 - Dashboard Analytics Integration', {
    tag: ['@loan', '@module', '@dashboard', '@integration'],
  }, async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/loans/dashboard');
    
    // Verify statistics cards
    await expect.soft(authenticatedPage.getByTestId('stat-active-loans')).toBeVisible();
    await expect.soft(authenticatedPage.getByTestId('stat-pending-applications')).toBeVisible();
    
    // Verify charts load
    await expect.soft(authenticatedPage.getByTestId('loan-analytics-chart')).toBeVisible();
  });
  ...
  test('18 - Accessibility: Guest Loan Form (WCAG 2.2 AA)', {
  ...
  test('19 - Accessibility: Authenticated Dashboard (WCAG 2.2 AA)', {
  ...
  test('21 - Accessibility: Form Validation', {
    tag: ['@loan', '@accessibility', '@forms'],
  }, async ({ page }) => {
    await page.goto('/loan/apply');
    
    // Submit empty form
    const submitButton = page.getByRole('button', { name: /next|seterusnya|submit/i }).first();
  ...
  test('22 - Cross-Module Navigation (Admin)', {
    ...
    await expect(page.locator('h1')).toContainText(/Loan Applications|Permohonan Pinjaman/i);
  ...
  test('23 - Responsive Design Integration', {
    ...
    // Verify mobile menu
    const mobileMenu = page.locator('[data-testid="mobile-menu-button"]').or(
