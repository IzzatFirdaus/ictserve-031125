...
class AccessibilityTest extends DuskTestCase
{
    ...
    public function guest_loan_form_keyboard_navigation(): void
    {
        $this->browse(function (Browser $browser) {
            $browser->visit('/loans/guest/create')
                ->assertSee(__('loans.application_title')) // 'Permohonan Pinjaman Aset'
                ->assertSee(__('loans.applicant_information')) // 'Maklumat Pemohon'
    ...
    /**
     * Test language switcher accessibility (disabled in v3.6.0)
     * Requirements: 6.1, 7.3, 15.2, 1.5
     * Verifies Bahasa Melayu-only interface
     */
    #[Test]
    public function language_switcher_accessibility(): void
    {
        $this->browse(function (Browser $browser) {
            $browser->visit('/loans/guest/create')
                ...
                ->assertMissing('[aria-haspopup="menu"]')
                ->assertMissing('[role="menu"]')

                // Verify only Bahasa Melayu content is displayed
                ->assertSee(__('forms.applicant_name')) // 'Nama Pemohon'
                ->assertSee(__('forms.email_address')) // 'Alamat E-mel'
                ->assertSee(__('forms.phone_number')) // 'Nombor Telefon'
                ->assertSee(__('forms.submit_button')); // 'Hantar'
        });
    }
    ...
    public function authenticated_portal_accessibility(): void
    {
        $this->browse(function (Browser $browser) {
            $browser->loginAs($this->user)
                ...
                ->assertSee(__('loans.dashboard_title')) // 'Papan Pemuka Pinjaman'
