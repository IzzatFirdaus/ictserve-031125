...
- [x] 4. Helpdesk Module Implementation with Hybrid Support
  ...
  - [x] 4.1 Create hybrid ticket submission system
    ...
    - Build Livewire guest form component for public ticket creation (no authentication required)
    - Implement authenticated portal ticket submission with pre-filled user data
    - Add dynamic category selection, asset linking, and priority assignment
    - Implement file attachment support with virus scanning hooks and size limits
    - Create automatic ticket numbering system (`HD[YYYY][000001-999999]` format)
    - Support both guest fields (guest_name, guest_email, guest_phone) and authenticated user_id
    - Send confirmation email within 60 seconds with ticket number and status tracking link
    - _Requirements: 1.1, 1.2, 1.3, 11.1, 11.6, 22.2, 22.3_

  - [x] 4.2 Implement ticket management workflows

    - Create ticket assignment system to MOTAC divisions or external agencies (manual and automatic)
    - Build admin dashboard using Filament with ticket queue and SLA tracking
    - Implement ticket status lifecycle (OPEN → ASSIGNED → IN_PROGRESS → PENDING_USER → RESOLVED → CLOSED)
    - Add internal comments system for authenticated staff and admin users
    - Implement SLA tracking with automated escalation within 25% of breach time
    - Build notification system for ticket updates via email (guest and authenticated users)
    - _Requirements: 2.2, 2.5, 8.4, 10.1, 10.3, 13.1-13.5, 22.3_

  - [x] 4.3 Build helpdesk reporting and analytics
    - Create unified dashboard showing ticket statistics and KPIs (volume, resolution time, SLA compliance)
    - Implement report generation for ticket volume, resolution times, and agent performance
    - Build analytics covering both guest and authenticated submissions
    - Add data export functionality (CSV, PDF, Excel) with proper headers and metadata
    - _Requirements: 3.2, 3.5, 3.6, 8.1, 8.2, 8.5_

- [x] 5. Asset Loan Module Implementation with Dual Approval

  - [x] 5.1 Create asset inventory management system

    - Build Filament resources for asset CRUD operations with categorization
    - Implement asset specification tracking (model, serial number, condition, location)
    - Create asset availability calendar using Livewire with booking visualization
    - Add asset condition tracking and maintenance history with automatic ticket creation for damage
    - Implement asset utilization reporting and analytics
    - _Requirements: 2.3, 3.1, 3.3, 7.1_

  - [x] 5.2 Implement hybrid loan application workflow with dual approval

    - Create guest loan application form (no authentication) with asset selection wizard
    - Build authenticated portal loan application with pre-filled user data
    - Implement approval matrix based on applicant grade and asset value
    - Generate secure approval tokens (7-day validity) for email-based approval
    - Build dual approval workflow: email-based (no login) AND portal-based (with login)
    - Send approval request emails to Grade 41+ officers with both approval options
    - Implement approval decision tracking (approval_method: email/portal, approval_remarks)
    - Send status update emails within 60 seconds to applicants (guest or authenticated)
    - Add loan extension request functionality for authenticated users
    - _Requirements: 1.4, 1.5, 1.6, 2.1, 2.2, 6.2, 10.1, 10.4, 12.1-12.5, 22.2, 23.2_

  - [x] 5.3 Build asset transaction management
    - Create asset check-out process with condition assessment and accessory tracking
    - Implement asset check-in process with return condition verification
    - Build overdue asset tracking with automated reminders (48 hours before, on due date, daily for overdue)
    - Add damage reporting with automatic maintenance ticket creation within 5 seconds
    - Implement transaction history and audit trail for all asset movements
...
- [-] 7. Unified Frontend Component Library and Design System
  ...
  - [x] 7.3 Implement session/cookie-only language switcher (NO user profile storage)
    ...
    - [x] 7.3.8 Documentation - Update D12, D14, and component library documentation
      ...
      - _Requirements: 17.1, 17.2, 17.3, 17.4, 17.5, 18.4_
                                                                                        ...
                                                                                        **Estimated Time:** 4-7 hours total (Backend 1-2h, Frontend 2-3h, Testing 1-2h)
                                                                                        **Requirements Covered:** 20.1-20.5, 7.2, 7.3, 14.4, 15.1-15.4, 6.1-6.5, 17.1-17.5, 18.4
                                                                                        **WCAG 2.2 Level:** AA (SC 1.4.3, 2.1.1, 2.4.7, 2.5.5, 3.1.2)
                                                                                        **Priority:** HIGH (Core accessibility and user experience feature)
...
- [-] 14. D00-D15 Compliance Assessment & Component Standardization
...
- [ ] 15. Final Hybrid Architecture Integration
...
- [ ] 18. Deployment and Production Setup
  ...
  - [ ] 18.3 Conduct user acceptance testing and training
    ...
    - \_Requirements: User adoption and system success_audit trail systems
    - _Requirements: 16.2, 17.2, 18.2_
