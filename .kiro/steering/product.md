...
- Approver workflows: Grade 41+ officers approve via signed email tokens (no system login)
...
**Backend**: Laravel 12.43.1, PHP 8.4.11 with strict typing  
**Frontend**: Livewire 3.7.3, Volt 1.10.1, Tailwind 4.0.0-alpha.25 (@theme config)  
**Admin Panel**: Filament 4.3.1 with role-based access control  
**Real-Time**: Laravel Reverb 1.6.3 + Echo 2.2.6 for WebSocket communication  
**Database**: MySQL 8.0 with nullable `user_id` foreign keys for hybrid architecture  
**AI Services**: Ollama 1.1 (local LLM) + AWS Bedrock (Claude models, Nova, Titan)  
**AI Infrastructure**: Redis (caching), Laravel Horizon 5.41.0 (queue), Laravel Pulse 1.4.7 (monitoring)  
**Queue**: Laravel Horizon 5.41.0 with Redis driver  
**Monitoring**: Laravel Pulse 1.4.7 (performance), Laravel Telescope 5.16.0 (debugging)
...
**Testing**: PHPUnit 11 (minimum 80% coverage)  
