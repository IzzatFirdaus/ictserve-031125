...
<div
    {{ $attributes->merge(['class' => 'flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg']) }}
    role="listitem">
    <span class="text-sm text-slate-700 dark:text-slate-300">
        {{ $label }}
    </span>
    <kbd class="inline-flex items-center gap-1 px-2 py-1 text-xs font-semibold text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-600 border border-slate-300 dark:border-slate-500 rounded-sm shadow-sm">
        @php
        $keys = explode(' + ', $key);
        @endphp
