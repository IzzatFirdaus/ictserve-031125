...
<div {{ $attributes->merge(['class' => "flex flex-col items-center justify-center rounded-lg $variantClasses $sizeClasses"]) }}
    ...
    @else
        ...
        @if ($actionText && ($actionUrl || $actionWire))
            @if ($actionUrl)
                <a href="{{ $actionUrl }}"
                    class="inline-flex items-center justify-center min-h-11 px-6 py-2.5 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900 shadow-sm"
