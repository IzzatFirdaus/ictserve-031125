<x-filament-panels::page>
    <div class="flex flex-col items-center justify-start min-h-[70vh] py-12 px-4 relative overflow-hidden">
        ...
        <div class="hidden md:block absolute top-0 left-1/2 transform -translate-x-1/2 -mt-12 opacity-5 pointer-events-none z-0" aria-hidden="true">
            <x-heroicon-o-magnifying-glass aria-hidden="true" class="w-48 sm:w-64 md:w-80 lg:w-96 h-auto text-gray-900 dark:text-white" />
        </div>
        ...
        <div class="w-full max-w-5xl relative z-10">
            ...
            <div class="relative w-full max-w-3xl mx-auto mb-12">
                <div class="group relative flex items-center px-6 py-5 bg-white dark:bg-gray-900 border-2 border-gray-200 dark:border-gray-700 rounded-2xl shadow-sm hover:shadow-md hover:border-primary-500 dark:hover:border-primary-500 transition-all duration-300">
                    <x-heroicon-o-magnifying-glass aria-hidden="true" class="w-7 h-7 text-gray-400 group-hover:text-primary-500 transition-colors mr-4 flex-shrink-0" />

                        <input type="text"
                        aria-label="{{ __('unified_search.input_label') }}"
                        wire:model.live.debounce.300ms="search"
                        placeholder="{{ __('unified_search.placeholder') }}"
                        class="flex-1 border-none focus:ring-0 p-0 text-xl text-gray-900 dark:text-white placeholder-gray-400 bg-transparent"
                        autofocus
                        autocomplete="off"
                        x-data
                ...
                @if ($search && strlen($search) >= 2)
                        ...
                        <div class="space-y-8 animate-fade-in-up">
                                ...
                                <x-filament::section>
                                    <x-slot name="heading">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <x-filament::badge color="gray">{{ count($results['tickets']) }}</x-filament::badge>
                                        </div>
                                    </x-slot>

                                    <div class="divide-y divide-gray-200 dark:divide-gray-700">
                                        @foreach ($results['tickets'] as $ticket)
                                            <a href="{{ $ticket['url'] }}" class="block py-4 hover:bg-gray-50 dark:hover:bg-white/5 -mx-6 px-6 transition-colors">
                                                <div class="flex items-start justify-between gap-4">
                                                    <div class="min-w-0 flex-1">
                                                        ...
                                                        <div class="flex items-center gap-3 mt-3">
                                                            ...
                                                            <span class="text-xs text-gray-400">{{ $ticket['metadata']['created_at'] }}</span>
                                                        </div>
                                                    </div>
                                                    <x-heroicon-m-chevron-right aria-hidden="true" class="w-5 h-5 text-gray-400" />
                                                </div>
                                            </a>
                                        @endforeach
                                    </div>
                            ...
                            @if (isset($results['loans']) && count($results['loans']) > 0)
                                <x-filament::section>
                                    <x-slot name="heading">
                                        <div class="flex items-center gap-2">
                                            <x-heroicon-o-document-text aria-hidden="true" class="w-5 h-5 text-primary-600" />
                                            <span>Loan Applications</span>
                                            <x-filament::badge color="gray">{{ count($results['loans']) }}</x-filament::badge>
                                        </div>
                                    </x-slot>

                                    <div class="divide-y divide-gray-200 dark:divide-gray-700">
                                        @foreach ($results['loans'] as $loan)
                                            <a href="{{ $loan['url'] }}" class="block py-4 hover:bg-gray-50 dark:hover:bg-white/5 -mx-6 px-6 transition-colors">
                                                <div class="flex items-start justify-between gap-4">
                                                    <div class="min-w-0 flex-1">
                                                        <h4 class="text-base font-semibold text-gray-900 dark:text-white truncate">
                                                            {{ $loan['title'] }}
                                                        ...
                                                        <div class="flex items-center gap-3 mt-3">
                                                            ...
                                                            <span class="text-xs text-gray-400">{{ $loan['metadata']['assets_count'] }} Assets</span>
                                                        </div>
                                                    </div>
                                                    <x-heroicon-m-chevron-right aria-hidden="true" class="w-5 h-5 text-gray-400" />
                                                </div>
                            ...
                            @if (isset($results['assets']) && count($results['assets']) > 0)
                                <x-filament::section>
                                    <x-slot name="heading">
                                        <div class="flex items-center gap-2">
                                            <x-heroicon-o-computer-desktop aria-hidden="true" class="w-5 h-5 text-primary-600" />
                                            <span>Assets</span>
                                            <x-filament::badge color="gray">{{ count($results['assets']) }}</x-filament::badge>
                                        </div>
                                    </x-slot>

                                    <div class="divide-y divide-gray-200 dark:divide-gray-700">
                                        @foreach ($results['assets'] as $asset)
                                            <a href="{{ $asset['url'] }}" class="block py-4 hover:bg-gray-50 dark:hover:bg-white/5 -mx-6 px-6 transition-colors">
                                                <div class="flex items-start justify-between gap-4">
                                                    <div class="min-w-0 flex-1">
                                                        <h4 class="text-base font-semibold text-gray-900 dark:text-white truncate">
                                                            {{ $asset['title'] }}
                                                        ...
                                                        <div class="flex items-center gap-3 mt-3">
                                                            ...
                                                            @if (isset($asset['metadata']['category']))
                                                                <span class="text-xs text-gray-400">{{ $asset['metadata']['category'] }}</span>
                                                            @endif
                                                        </div>
                                                    </div>
                                                    <x-heroicon-m-chevron-right aria-hidden="true" class="w-5 h-5 text-gray-400" />
                             ...
                             <x-filament::section>
                                 <x-slot name="heading">
                                     <div class="flex items-center gap-2">
                                         ...
                                         <span>Users</span>
                                         <x-filament::badge color="gray">{{ count($results['users']) }}</x-filament::badge>
                                     </div>
                                 </x-slot>

                                 <div class="divide-y divide-gray-200 dark:divide-gray-700">
                                     @foreach ($results['users'] as $user)
                                         <a href="{{ $user['url'] }}" class="block py-4 hover:bg-gray-50 dark:hover:bg-white/5 -mx-6 px-6 transition-colors">
                                             <div class="flex items-start justify-between gap-4">
                                                 <div class="min-w-0 flex-1">
                                                     <h4 class="text-base font-semibold text-gray-900 dark:text-white truncate">
                                                         {{ $user['title'] }}
                    ...
                    @else
                        ...
                        <div class="flex flex-col items-center justify-center py-16 text-center">
                            ...
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">{{ __('unified_search.no_results_title') }}</h3>
