...
<header class="bg-slate-900 text-slate-100 border-b border-slate-800" role="banner"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div class="flex items-center gap-8">
            ...
            <a href="{{ route('staff.dashboard') }}"
                class="flex items-center gap-3 py-3 focus:outline-none focus-visible:ring-3 focus-visible:ring-offset-2 focus-visible:ring-blue-500 focus:ring-offset-slate-900 rounded-lg transition-colors duration-150"
            ...
            <nav id="sidebar-navigation" class="hidden md:flex items-center gap-6 text-sm font-medium" role="navigation"
                ...
                @foreach ($this->navigationLinks as $link)
                    <a wire:key="nav-{{ $link['route'] }}" href="{{ route($link['route']) }}"
                        ...
                        @else
                            wire:navigate @endif
                        class="px-4 py-3 border-b-2 min-h-11 flex items-center transition-colors duration-150 focus:outline-none focus-visible:ring-3 focus-visible:ring-offset-2 focus-visible:ring-blue-500 focus:ring-offset-slate-900 rounded-lg {{ $this->isCurrentRoute($link['route']) ? 'border-blue-500 text-white font-semibold' : 'border-transparent text-slate-300 hover:text-white hover:border-slate-500' }}"
        ...
        <div class="flex items-center gap-4" id="user-menu">
            ...
            <x-dropdown align="right" width="48">
                <x-slot name="trigger">
                    <button
                        class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-lg bg-slate-800 text-slate-200 hover:bg-slate-700 focus:outline-none focus-visible:ring-3 focus-visible:ring-blue-500 focus:ring-offset-2 focus:ring-offset-slate-900 transition-colors duration-150 min-h-11"
                        aria-haspopup="menu"
    ...
    <nav x-data="{ mobileMenuOpen: false }" class="md:hidden border-t border-slate-800" role="navigation"
        ...
        <button type="button"
            class="w-full px-4 py-3 flex items-center justify-between text-sm font-medium text-slate-200 bg-slate-900 hover:bg-slate-800 focus:outline-none focus-visible:ring-3 focus-visible:ring-blue-500 focus:ring-inset transition-colors duration-150 min-h-11"
