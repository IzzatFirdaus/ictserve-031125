                ...
                <div>
                    ...
                    <select wire:model.live="statusFilter" id="status-filter" name="status"
                        ...
                        @foreach ($this->availableStatuses as $value => $label)
                            <option wire:key="history-status-{{ $value }}" value="{{ $value }}">
                                {{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select wire:model.live="categoryFilter" id="category-filter" name="category"
                        ...
                        @foreach ($this->availableCategories as $value => $label)
                            <option wire:key="history-category-{{ $value }}" value="{{ $value }}">
                                {{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <div>
                        ...
                        <select wire:model.live="priorityFilter" id="priority-filter" name="priority"
                            ...
                            @foreach ($this->availablePriorities as $value => $label)
                                <option wire:key="history-priority-{{ $value }}" value="{{ $value }}">
                                    {{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                            ...
                            @foreach ($this->savedSearches as $search)
                                <div wire:key="saved-search-{{ $search['id'] }}" class="py-1">
                                    <div
                                        ...
                                        <button wire:click="applySavedSearch({{ $search['id'] }})"
                ...
                @if (!$this->hasActiveFilters)
                    <div class="mt-6">
                        <a href="{{ $activeTab === 'helpdesk' ? route('helpdesk.create') : route('loans.create') }}"
                            class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-primary-600 hover:bg-primary-700 focus:outline-none focus-visible:ring-3 focus-visible:ring-primary-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-gray-900 min-w-11 min-h-11">
            ...
            <div class="space-y-4 sm:hidden" role="list" aria-label="{{ __('portal.history_title') }}">
                @foreach ($this->submissions as $submission)
                    <article wire:key="submission-card-{{ $loop->iteration }}-{{ $submission->id }}"
                        ...
                        role="listitem">
                        <h3
                            class="text-sm font-semibold text-gray-900 dark:text-gray-100 flex items-center justify-between">
                            <span>
                        ...
                        <dl class="mt-3 space-y-2 text-sm text-gray-700 dark:text-gray-200">
                            @if ($activeTab === 'helpdesk')
                                <div class="flex justify-between">
                                    ...
                                    <dd class="text-right">
                                        {{ Str::limit($submission->subject ?? $submission->description, 50) }}</dd>
                                </div>
                                <div class="flex justify-between">
                                    <dt class="font-medium">{{ __('portal.category') }}</dt>
                                    <dd class="text-right">{{ __('helpdesk.categories.' . $submission->category) }}
                                    </dd>
                                </div>
                                <div class="flex justify-between">
                                    <dt class="font-medium">{{ __('portal.priority') }}</dt>
                                    <dd class="text-right">{{ __('helpdesk.priorities.' . $submission->priority) }}
                                    </dd>
                                </div>
                            @else
                                <div class="flex justify-between">
                                ...
                                <div class="flex justify-between">
                                    ...
                                    <dd class="text-right">{{ $submission->start_date->format('d/m/Y') }} -
                                        {{ $submission->end_date->format('d/m/Y') }}</dd>
                                </div>
                            @endif
                            <div class="flex justify-between">
                                <dt class="font-medium">{{ __('portal.status') }}</dt>
                                <dd class="text-right">
                                    {{ $activeTab === 'helpdesk' ? __('helpdesk.statuses.' . $submission->status) : __('loans.statuses.' . $submission->status) }}
                                </dd>
                            </div>
                            <div class="flex justify-between">
                                <dt class="font-medium">
                                    {{ $activeTab === 'helpdesk' ? __('portal.created_on') : __('portal.requested_on') }}
                                </dt>
                                ...
                                @else
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-900 dark:text-gray-100">
                                        ...
                                        @if ($submission->items->count() > 1)
                                            <span
                                                class="text-gray-500 dark:text-gray-400">+{{ $submission->items->count() - 1 }}</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        ...
                                        @endphp
                                        <span
                                            class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $loanStatusClasses }}">
                ...
                <div
                    ...
                    <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            <div
                                ...
                                <x-heroicon-o-inbox-arrow-down class="h-6 w-6 text-primary-600 dark:text-primary-400"
                                    aria-hidden="true" />
                            </div>
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                                ...
                                <div class="mt-4">
                                    ...
                                    <input type="text" wire:model="savedSearchName" id="search-name"
                                        name="search_name" maxlength="50"
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button wire:click="saveSearch" type="button"
                            class="w-full inline-flex justify-center rounded-lg border border-transparent shadow-sm px-4 py-2 bg-primary-600 text-base font-medium text-white hover:bg-primary-700 focus:outline-none focus-visible:ring-3 focus-visible:ring-primary-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-gray-900 sm:ml-3 sm:w-auto sm:text-sm min-w-11 min-h-11">
                            {{ __('portal.save') }}
                        </button>
                        <button wire:click="closeSaveSearchModal" type="button"
                            class="mt-3 w-full inline-flex justify-center rounded-lg border border-gray-300 dark:border-gray-600 shadow-sm px-4 py-2 bg-white dark:bg-gray-700 text-base font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus-visible:ring-3 focus-visible:ring-primary-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-gray-900 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm min-w-11 min-h-11">
                            {{ __('portal.cancel') }}
                        </button>
                    </div>
                </div>
