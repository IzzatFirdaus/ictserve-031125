<div class="bg-gradient-to-br from-gray-900 via-slate-900 to-gray-900 min-h-screen flex flex-col">
    <!-- Header -->
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    <!-- MOTAC Logo -->
                    <img src="{{ asset('images/motac-logo.jpeg') }}" alt="Logo MOTAC" class="h-12 w-auto">
                    <!-- BPM Logo -->
                    <img src="{{ asset('images/bpm-logo.png') }}" alt="Logo BPM" class="h-12 w-auto">
    ...
    <div class="flex-1 flex">
        ...
        <div class="{{ $showSidebar ? 'w-64' : 'w-0' }} transition-all duration-300 bg-slate-900 border-r border-orange-900/30 overflow-hidden">
            <div class="p-4">
                <button wire:click="newConversation" class="w-full px-4 py-2 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white rounded-lg mb-4 font-semibold shadow-lg shadow-orange-500/20 min-h-[44px]">
                    + New Chat
                </button>
                <div class="space-y-2">
                    @foreach($conversations as $conv)
                        <div class="group flex items-center gap-2 p-2 rounded hover:bg-slate-800 cursor-pointer transition-colors">
                            ...
                            <button wire:click="deleteConversation({{ $conv->id }})" class="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-500 transition-opacity min-h-[44px] min-w-[44px]">
                                ×
                            </button>
                        </div>
                    @endforeach
        ...
        <div class="flex-1 p-6">
            <div class="bg-slate-800 rounded-lg shadow-2xl shadow-orange-500/10 p-6 border border-orange-900/30">
                <div class="flex items-center justify-between mb-6">
                    <button wire:click="$toggle('showSidebar')" class="p-2 hover:bg-slate-700 rounded-lg transition-colors text-gray-300 min-h-[44px] min-w-[44px]">
                ...
                <div class="mb-6 h-96 overflow-y-auto border border-orange-900/30 rounded-lg p-4 bg-slate-900/50 backdrop-blur-sm">
                    @forelse($messages as $message)
                        <div class="mb-4 {{ $message['role'] === 'user' ? 'text-right' : 'text-left' }}">
                            <div class="inline-block max-w-[80%] p-4 rounded-lg shadow-lg {{ $message['role'] === 'user' ? 'bg-gradient-to-r from-orange-500 to-orange-600 text-white' : 'bg-slate-800 border border-orange-900/30 text-gray-100' }}">
                ...
                <form wire:submit="send" class="flex gap-3">
                    ...
                    <button 
                        type="submit" 
                        class="px-8 py-3 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white rounded-lg font-semibold shadow-lg shadow-orange-500/20 disabled:opacity-50 disabled:cursor-not-allowed transition-all min-h-[44px]"
    ...
    <footer class="bg-slate-900 border-t border-orange-500/20 mt-auto" role="contentinfo">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
                <div class="flex items-center space-x-4">
                    <img src="{{ asset('images/motac-logo.jpeg') }}" alt="Logo MOTAC" class="h-8 w-auto">
                    <img src="{{ asset('images/bpm-logo.png') }}" alt="Logo BPM" class="h-8 w-auto">
                </div>
