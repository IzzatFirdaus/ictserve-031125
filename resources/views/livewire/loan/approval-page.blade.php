...
$approve = function () {
    ...
    try {
        ...
        if (class_exists(LoanApplicationDecision::class)) {
            Mail::to($this->application->applicant_email)
                ->queue(new LoanApplicationDecision($this->application, true));
        }
...
$reject = function () {
    ...
    try {
        ...
        if (class_exists(LoanApplicationDecision::class)) {
            Mail::to($this->application->applicant_email)
                ->queue(new LoanApplicationDecision($this->application, false));
        }
...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-3xl mx-auto">
        ...
        {{-- Success Message --}}
        @if($success)
        <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-6 mb-6" role="alert">
            <div class="flex items-start">
                ...
                <div class="ml-3">
                    ...
                    <p class="mt-1 text-sm text-green-700 dark:text-green-300">{{ $success }}</p>
                    <div class="mt-4">
                        <a href="{{ url('/') }}"
                           class="inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">
        ...
        {{-- Error Messages --}}
        @if($error)
        <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6 mb-6" role="alert">
            <div class="flex items-start">
                <div class="shrink-0">
                ...
                <div class="ml-3">
                    ...
                    <div class="mt-4">
                        <a href="{{ url('/') }}"
                           class="inline-flex items-center px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white text-sm font-medium rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2">
                            {{ __('common.back_to_home') }}
                        </a>
                    </div>
                </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden" role="region" aria-labelledby="application-summary-heading">
                <div class="bg-blue-600 dark:bg-blue-700 px-6 py-4">
                    ...
                    <p class="text-blue-100 text-sm mt-1">
                        {{ __('loan.approval.reference') }}: <span class="font-mono font-semibold">{{ $application->application_number }}</span>
                    </p>
                </div>
                ...
                <div class="px-6 py-5">
                    ...
                    <div class="mb-6">
                        ...
                        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('loan.fields.applicant_name') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white font-medium">{{ $application->applicant_name }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('loan.fields.email') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $application->applicant_email }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('loan.fields.staff_id') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $application->staff_id ?? 'N/A' }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('loan.fields.division') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $application->division?->name ?? 'N/A' }}</dd>
                            </div>
                        </dl>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <dl class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('loan.fields.start_date') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $application->loan_start_date?->translatedFormat('d M Y') ?? 'N/A' }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('loan.fields.end_date') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $application->loan_end_date?->translatedFormat('d M Y') ?? 'N/A' }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('loan.fields.duration') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $this->loanDuration }} {{ __('common.days') }}</dd>
                            </div>
                        </dl>
                    </div>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
                        <div class="flex justify-between items-center">
                            <span class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('loan.fields.total_value') }}</span>
                            <span class="text-lg font-bold text-gray-900 dark:text-white">RM {{ number_format($application->total_value, 2) }}</span>
                        </div>
                    </div>
                    @endif
                </div>
            ...
            {{-- Decision Form --}}
            <div class="bg-white dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden" role="region" aria-labelledby="decision-form-heading">
                ...
                <div class="px-6 py-5">
                    ...
                    <div class="mb-6">
                        <label for="remarks" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            ...
                            <span class="text-gray-400 text-xs ml-1">({{ __('loan.approval.required_for_rejection') }})</span>
                        </label>
                        <textarea
                            id="remarks"
                            wire:model="remarks"
                            rows="4"
                            class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors resize-none"
                            placeholder="{{ __('loan.approval.remarks_placeholder') }}"
                    ...
                    <div class="flex flex-col sm:flex-row gap-4">
                        ...
                        <button
                            ...
                            class="flex-1 inline-flex items-center justify-center px-6 py-3 bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white font-semibold rounded-lg transition-colors focus:outline-none focus:ring-4 focus:ring-green-300 dark:focus:ring-green-800"
                        ...
                        <button
                            ...
                            wire:target="reject"
                            @if($isProcessing) disabled @endif
                            class="flex-1 inline-flex items-center justify-center px-6 py-3 bg-red-600 hover:bg-red-700 disabled:bg-red-400 text-white font-semibold rounded-lg transition-colors focus:outline-none focus:ring-4 focus:ring-red-300 dark:focus:ring-red-800"
                    ...
                    {{-- Security Notice --}}
                    <div class="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg" role="note">
                        <div class="flex">
                            <div class="shrink-0">
