...
<div class="max-w-4xl mx-auto p-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
        ...
        @if (count($this->missingFields) > 0)
            <div
                class="mt-4 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-md">
    ...
    <form wire:submit="updateProfile" class="space-y-6">
                ...
                <div class="flex-1">
                    ...
                    @if ($currentAvatar)
                        <button type="button" wire:click="removeAvatar"
                            class="mt-2 text-sm text-red-600 hover:text-red-800">{{ __('portal.remove_photo') }}</button>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="name" wire:model.live.debounce.300ms="name"
                        class="mt-1 block w-full px-4 py-2 min-h-11 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md"
                        required>
                    @error('name')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <textarea id="bio" wire:model.live.debounce.300ms="bio" rows="4" maxlength="1000"
                    class="mt-1 block w-full px-4 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md"></textarea>
        ...
        <div class="flex justify-end gap-4">
            <button type="submit"
                class="inline-flex items-center px-6 py-3 min-h-11 min-w-11 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
