...
<div class="space-y-8 text-slate-100" aria-describedby="loan-dashboard-description">
    ...
    <header class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <x-ui.button icon="heroicon-o-arrow-path" wire:click="refreshData" wire:loading.attr="disabled"
            ...
            <x-ui.card variant="portal" aria-label="{{ __('loans.overdue_items') }}">
                <div class="flex items-center justify-between">
                    ...
                    <div class="rounded-full border border-danger-500/30 bg-danger-500/10 p-3 text-danger-300">
                        <svg class="h-6 w-6 text-danger-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            aria-hidden="true">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <section id="loan-tabs" aria-label="{{ __('loans.loan_management_tabs') }}">
        <x-navigation.tabs :tabs="[
            ...
            [
                'id' => 'active',
                'label' => __('loans.tab_active_loans'),
                'icon' => 'check-circle',
                'badge' => $this->activeLoansCount,
            ],
        ...
        ]" :active-tab="$activeTab" wire:model.live="activeTab" />
                    ...
                    @if ($this->activeLoans->count() > 0)
                        <x-ui.card variant="portal">
                            ...
                            <div class="space-y-4" role="feed"
                                aria-label="{{ __('loans.recent_active_loans') }}">
                                @foreach ($this->activeLoans->take(3) as $loan)
                                    <div class="flex items-start gap-4 border-l-4 border-primary-500/50 bg-slate-900/40 p-4 rounded-r-lg hover:bg-slate-900/60 transition-colors"
                                        role="article">
                                        <div class="shrink-0">
                                            <div
                                                class="flex h-10 w-10 items-center justify-center rounded-full border border-primary-500/30 bg-primary-500/10">
                                                <svg class="h-5 w-5 text-primary-400" fill="none"
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p class="text-sm font-medium text-slate-200">
                                                <a href="{{ route('loan.authenticated.show', $loan->id) }}"
                                                    class="text-slate-200 hover:text-primary-400 transition-colors">
                                                    {{ $loan->application_number }}
                                                </a>
                                            </p>
                                            <p class="mt-1 text-sm text-slate-400">
                                                {{ $loan->loanItems->count() }} {{ __('loans.items') }} •
                                                {{ optional($loan->loan_start_date)->translatedFormat('d M Y') ?? '-' }}
                                                -
                                                {{ optional($loan->loan_end_date)->translatedFormat('d M Y') ?? '-' }}
                                            </p>
                                            <div class="mt-2">
                                                <span
                            ...
                            @if ($this->activeLoans->count() > 3)
                                <div class="mt-4 text-center">
                                    <button wire:click="switchTab('active')"
                                        class="text-sm font-medium text-primary-300 hover:text-primary-400 min-h-11 inline-flex items-center">
                                ...
                                <thead class="bg-slate-900/40">
                                    <tr>
                                        <th scope="col"
                                            ...
                                            {{ __('loans.application_number') }}</th>
                                        <th scope="col"
                                            class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-slate-400">
                                            {{ __('loans.items') }}</th>
                                        <th scope="col"
                                        ...
                                        <th scope="col"
                                            ...
                                            {{ __('loans.status') }}</th>
                                        <th scope="col"
                                            class="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-slate-400">
                                            {{ __('loans.actions') }}</th>
                                    </tr>
                                </thead>
                                        ...
                                        <tr>
                                            <td class="px-4 py-4 text-sm text-slate-100">
                                                <div class="font-medium text-slate-200">
                                                    {{ $loan->application_number }}</div>
                                                <div class="text-xs text-slate-400">
                                                    {{ optional($loan->created_at)->translatedFormat('d M Y') ?? '-' }}
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-4 py-4 text-sm text-right">
                                                <a href="{{ route('loan.authenticated.show', $loan->id) }}"
                                                    class="font-medium text-primary-300 hover:text-primary-400 min-h-11 inline-flex items-center">{{ __('loans.view_details') }}</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
            ...
            @elseif($activeTab === 'pending')
                <x-ui.card variant="portal">
                    ...
                    @if ($this->pendingApplications->count() > 0)
                        <ul class="space-y-4" aria-label="{{ __('loans.pending_applications_list') }}">
                            @foreach ($this->pendingApplications as $application)
                                <li
                                    class="flex items-start gap-4 border-l-4 border-warning-500/50 bg-slate-900/40 p-4 rounded-r-lg hover:bg-slate-900/60 transition-colors">
                                    <div class="shrink-0">
                                        <div
                                            class="flex h-10 w-10 items-center justify-center rounded-full border border-warning-500/30 bg-warning-500/10">
                                            <svg class="h-5 w-5 text-warning-400" fill="none"
                                                stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                            </svg>
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-medium text-slate-200">
                                            <a href="{{ route('loan.authenticated.show', $application->id) }}"
                                                class="text-slate-200 hover:text-primary-400 transition-colors">
                                                {{ $application->application_number }}
                                            </a>
                                        </p>
                                        <p class="mt-1 text-sm text-slate-400">
                                            {{ Str::limit($application->purpose, 100) }}</p>
                                        <div class="mt-2 flex items-center gap-4 text-xs text-slate-300">
                                            ...
                                            <span
                                                class="inline-flex items-center rounded-full border border-warning-500/30 bg-warning-500/10 px-2 py-0.5 text-xs font-medium text-warning-300">
                ...
                <x-ui.card variant="portal">
                    <div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div class="flex flex-col sm:flex-row gap-2">
                            <x-form.input name="search" wire:model.live.debounce.300ms="search" type="search"
                                placeholder="{{ __('loans.search_loans') }}" class="min-h-11"
                                ...
                                <tbody class="divide-y divide-slate-800 bg-slate-900/70">
                                    @foreach ($this->loanHistory as $loan)
                                        <tr>
                                            ...
                                            <td class="px-4 py-4 text-sm text-slate-200">
                                                {{ Str::limit($loan->purpose, 50) }}</td>
                                            <td class="px-4 py-4 text-sm text-slate-200">
                                                {{ optional($loan->loan_start_date)->translatedFormat('d M') ?? '-' }}
                                                -
                                                {{ optional($loan->loan_end_date)->translatedFormat('d M Y') ?? '-' }}
                                            </td>
                                            <td class="px-4 py-4 text-sm text-slate-200">
                                            ...
                                            <td class="px-4 py-4 text-sm text-right">
                                                <a href="{{ route('loan.authenticated.show', $loan->id) }}"
                                                    class="font-medium text-primary-300 hover:text-primary-400 min-h-11 inline-flex items-center">{{ __('loans.view') }}</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
