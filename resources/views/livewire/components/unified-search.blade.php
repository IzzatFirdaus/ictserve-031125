...
<div class="relative" x-data="{
    ...
    <div class="relative">
        ...
        <div class="relative">
            ...
            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                <x-heroicon-o-magnifying-glass class="h-5 w-5 text-gray-400 dark:text-gray-500" aria-hidden="true" />
            </div>
            ...
            <input x-ref="searchInput" type="search" id="unified-search" wire:model.live.debounce.300ms="query"
                ...
                class="block w-full rounded-lg border border-gray-300 bg-white py-2.5 pl-10 pr-10 text-sm text-gray-900 placeholder-gray-500 transition-colors duration-200 focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:placeholder-gray-400 dark:focus:border-primary-400 dark:focus:ring-primary-400/20"
        ...
        <div class="pointer-events-none absolute inset-y-0 right-8 hidden items-center pr-3 sm:flex">
            <kbd
                class="hidden rounded border border-gray-200 bg-gray-100 px-1.5 py-0.5 text-xs font-medium text-gray-500 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-400 sm:inline-block">
                /
            </kbd>
        </div>
    </div>
    ...
    @if ($isOpen && count($this->suggestions) > 0)
        <div id="search-suggestions" role="listbox"
            class="absolute z-50 mt-2 w-full rounded-lg border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800"
            ...
            <ul class="py-2">
                @foreach ($this->suggestions as $index => $suggestion)
                    <li id="suggestion-{{ $index }}" role="option"
                        ...
                        class="flex cursor-pointer items-center gap-3 px-4 py-2.5 transition-colors duration-150 {{ $selectedIndex === $index ? 'bg-primary-50 dark:bg-primary-900/20' : 'hover:bg-gray-50 dark:hover:bg-gray-700/50' }}">
                        {{-- Type Icon --}}
                        <span
                        ...
                        {{-- Arrow Icon --}}
                        <x-heroicon-o-arrow-right class="h-4 w-4 shrink-0 text-gray-400 dark:text-gray-500"
                            aria-hidden="true" />
                    </li>
                @endforeach
            </ul>
            ...
            <div class="border-t border-gray-200 px-4 py-2 dark:border-gray-700">
                <button type="button" wire:click="performSearch"
                    class="flex w-full items-center justify-center gap-2 rounded-md px-3 py-2 text-sm font-medium text-primary-600 transition-colors hover:bg-primary-50 dark:text-primary-400 dark:hover:bg-primary-900/20">
    ...
    @if ($showModal)
        <div x-data="{ show: true }" x-show="show" x-transition:enter="transition ease-out duration-300"
            ...
            aria-modal="true">
            {{-- Backdrop --}}
            <div class="fixed inset-0 bg-gray-500/75 transition-opacity dark:bg-gray-900/80" wire:click="closeModal"
            ...
            <div class="flex min-h-full items-start justify-center p-4 pt-16 sm:pt-24">
                <div x-show="show" x-transition:enter="transition ease-out duration-300"
                    ...
                    class="relative w-full max-w-2xl transform overflow-hidden rounded-xl bg-white shadow-2xl transition-all dark:bg-gray-800"
                        ...
                        @if ($this->results['total'] === 0)
                            ...
                            <div class="py-8 text-center">
                                <x-heroicon-o-magnifying-glass
                                    class="mx-auto h-12 w-12 text-gray-400 dark:text-gray-500" aria-hidden="true" />
                                            ...
                                            @foreach ($this->results['tickets'] as $ticket)
                                                <li>
                                                    <a href="{{ route('portal.tickets.show', $ticket) }}"
                                                        class="block rounded-lg border border-gray-200 p-3 transition-colors hover:border-primary-300 hover:bg-primary-50/50 dark:border-gray-700 dark:hover:border-primary-600 dark:hover:bg-primary-900/20">
                                                        <div class="flex items-start justify-between gap-3">
                                                            ...
                                                            <span
                                                                class="shrink-0 rounded-full px-2 py-1 text-xs font-medium {{ $ticket->status === 'open' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' : ($ticket->status === 'resolved' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300') }}">
