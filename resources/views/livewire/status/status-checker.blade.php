...
<div class="bg-slate-50 dark:bg-gray-900 min-h-screen">
    ...
    <section class="bg-primary-600 dark:bg-primary-700 text-white py-10 md:py-14 theme-transition" role="banner" aria-labelledby="status-heading">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center gap-4 mb-4">
                <div class="h-12 w-12 rounded-xl bg-white/15 flex items-center justify-center" aria-hidden="true">
                    <x-heroicon-o-magnifying-glass class="w-7 h-7" />
                </div>
                <div>
                    <p class="text-sm text-blue-100 font-semibold tracking-wide uppercase">{{ __('status.page_tagline') ?? 'Status Semasa' }}</p>
                    <h1 id="status-heading" class="text-3xl md:text-4xl font-heading font-bold tracking-tight">{{ __('status.title') }}</h1>
                </div>
            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-4 md:grid-cols-8 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
                <div class="col-span-4 md:col-span-8 lg:col-span-8">
                    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-card dark:shadow-dropdown rounded-2xl p-6 md:p-8 theme-transition">
                        <div class="flex items-start gap-3 mb-6">
                            <div class="h-10 w-10 rounded-full bg-primary-50 dark:bg-primary-900/40 flex items-center justify-center" aria-hidden="true">
                                <x-heroicon-o-key class="w-5 h-5 text-primary-600 dark:text-primary-300" />
                            </div>
                            <div>
                                <h2 id="form-heading" class="text-2xl font-heading font-semibold text-gray-900 dark:text-gray-100">{{ __('status.form_label') }}</h2>
                                <p class="text-sm text-gray-600 dark:text-gray-300 mt-1">{{ __('status.form_helper') ?? __('status.subtitle') }}</p>
                            </div>
                        </div>
                        ...
                        <form wire:submit.prevent="checkStatus" class="space-y-6" novalidate aria-label="{{ __('status.form_label') }}">
                            <div class="space-y-5">
                                {{-- Token Input --}}
                                <div>
                                    <label for="token" class="block text-sm font-medium text-gray-800 dark:text-gray-100 mb-1">
                                    ...
                                    @error('token')
                                        <p id="token-error" class="mt-1 text-sm text-danger-600 dark:text-danger-400" role="alert">
                                            {{ $message }}
                                        </p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    <label for="type" class="block text-sm font-medium text-gray-800 dark:text-gray-100 mb-1">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                                @if ($showResults)
                                    <p class="text-sm text-gray-600 dark:text-gray-300">
                                        ...
                                        <span class="font-medium text-gray-800 dark:text-gray-100">{{ now()->translatedFormat('d M Y, h:i A') }}</span>
                                    </p>
                                @else
                                    <div></div>
                                @endif
                    ...
                    {{-- Error Message --}}
                    @if ($notFound)
                        <div class="mt-6 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-700 p-4" role="alert" aria-live="polite">
                            <div class="flex">
                                <div class="shrink-0">
                    ...
                    @if ($showResults && $submission)
                        <div class="mt-8 space-y-6" aria-live="polite">
                            {{-- Submission Header --}}
                            <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-card dark:shadow-dropdown p-6">
                                <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                                    <div>
                                        <div class="flex items-center gap-2 mb-2">
                                            @if ($foundType === 'ticket')
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800 dark:bg-purple-900/40 dark:text-purple-100">
                                    ...
                                    <div class="shrink-0">
                                        @if ($foundType === 'ticket')
                                            @php
                                                $statusClass = match($submission->status) {
                                                    'open', 'new' => 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/40 dark:text-yellow-200',
                                                    'assigned', 'in_progress' => 'bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-200',
                                        ...
                                        @else
                                            <span class="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium bg-{{ $submission->status->color() }}-100 text-{{ $submission->status->color() }}-800 dark:bg-{{ $submission->status->color() }}-900/30 dark:text-{{ $submission->status->color() }}-100">
                                ...
                                <dl class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                    @if ($foundType === 'ticket')
                                        <div>
                                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('status.category') }}</dt>
                                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $submission->category?->name ?? __('status.not_specified') }}</dd>
                                        </div>
                                        <div>
                                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('status.priority') }}</dt>
                                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ \Illuminate\Support\Str::headline($submission->priority ?? 'normal') }}</dd>
                                        </div>
                                        <div>
                                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('status.division') }}</dt>
                                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $submission->division?->name ?? $submission->guest_division ?? __('status.not_specified') }}</dd>
                                        </div>
                                    @else
                                        <div>
                                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('status.applicant') }}</dt>
                                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $submission->applicant_name }}</dd>
                                        </div>
                                        <div>
                                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('status.loan_period') }}</dt>
                                        ...
                                        <div>
                                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('status.location') }}</dt>
                                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $submission->location ?? __('status.not_specified') }}</dd>
                                        </div>
                                    @endif
                                </dl>
                            </div>
                                        ...
                                        <li class="ml-6">
                                            @php
                                                $markerClass = $event['current']
                                                    ...
                                                    : ($event['completed'] ? 'border-blue-600 bg-white dark:bg-blue-900/40' : 'border-gray-300 bg-white dark:bg-gray-800');
                                            @endphp
                                            ...
                                            <div class="rounded-lg border p-4 transition-all {{ $cardClass }}">
                                                <h4 class="text-base font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-2 flex-wrap">
                                                    {{ $event['label'] }}
                                                    @if ($event['current'])
                                    ...
                                    <div class="space-y-4">
                                        @foreach ($publicComments as $comment)
                                            <div class="bg-gray-50 dark:bg-gray-900/40 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                                                ...
                                                <p class="text-sm text-gray-700 dark:text-gray-200">{{ $comment['content'] }}</p>
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                                ...
                                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-card dark:shadow-dropdown p-6">
                                    ...
                                    <ul class="divide-y divide-gray-200 dark:divide-gray-700 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                                        @foreach ($submission->loanItems as $item)
                                            <li class="px-4 py-3 flex justify-between items-center bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700/60">
                                                <div class="flex items-center gap-3">
                                                    <x-heroicon-o-computer-desktop class="w-5 h-5 text-gray-400 dark:text-gray-500" />
                            ...
                            @if ($foundType === 'ticket' && $submission->description)
                                <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-card dark:shadow-dropdown p-6">
                                    ...
                                    <div class="rounded-lg bg-gray-50 dark:bg-gray-900/40 border border-gray-200 dark:border-gray-700 p-4 text-gray-700 dark:text-gray-200">
                                        {{ $submission->description }}
                                    </div>

                                    @if ($submission->resolution_notes)
                                        <div class="mt-4 rounded-lg bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-700 p-4">
                                            <p class="text-sm font-semibold text-emerald-700 dark:text-emerald-200 mb-2">
                                                {{ __('status.resolution_notes') }}
                                            </p>
                                            <p class="text-sm text-emerald-800 dark:text-emerald-100">{{ $submission->resolution_notes }}</p>
                                        </div>
                                    @endif
                                </div>
                            @endif
                    ...
                    <div class="mt-10 text-center">
                        <p class="text-sm text-gray-600 dark:text-gray-300">
                            {{ __('status.help_text') }}
                            <a href="{{ route('helpdesk.create') }}" class="text-primary-600 dark:text-primary-300 font-semibold hover:underline focus:outline-none focus:ring-3 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900 rounded-sm">
                ...
                <aside class="col-span-4 md:col-span-8 lg:col-span-4 space-y-4">
                    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-card dark:shadow-dropdown p-6">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-3">{{ __('status.quick_help_title') ?? 'Bantuan Pantas' }}</h3>
                        <ul class="space-y-3 text-sm text-gray-700 dark:text-gray-200">
                            <li class="flex items-start gap-2">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-600 dark:text-gray-300">{{ __('status.quick_help_email') ?? 'Emel sokongan BPM' }}</p>
                                </div>
                            </li>
                            <li class="flex items-start gap-2">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-600 dark:text-gray-300">{{ __('status.quick_help_phone') ?? 'Talian bantuan helpdesk' }}</p>
                                </div>
                            </li>
                            <li class="flex items-start gap-2">
                                <x-heroicon-o-information-circle class="w-5 h-5 text-primary-600 dark:text-primary-300 mt-0.5" />
                                <div>
                                    <p class="font-semibold">{{ __('status.quick_help_ticket') ?? 'Hantar tiket baharu' }}</p>
                                    <a href="{{ route('helpdesk.submit') }}" class="text-primary-600 dark:text-primary-300 font-semibold hover:underline focus:outline-none focus:ring-3 focus:ring-primary-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900 rounded-sm">{{ __('status.quick_help_ticket_cta') ?? 'Pergi ke borang helpdesk' }}</a>
                                </div>
                            </li>
                        </ul>
                    </div>
