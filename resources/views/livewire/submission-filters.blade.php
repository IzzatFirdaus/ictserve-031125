...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        @if ($this->hasActiveFilters)
            <button type="button" wire:click="clearFilters"
                class="inline-flex items-center px-4 py-2 min-h-11 min-w-11 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus-visible:ring-3 focus-visible:ring-primary-500 focus-visible:ring-offset-2 transition-colors duration-200"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="space-y-2">
            ...
            <div class="relative" x-data="{ open: false }" x-cloak>
                ...
                <button type="button" @click="open = !open" @click.away="open = false"
                    class="w-full inline-flex items-center justify-between px-4 py-2 min-h-11 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus-visible:ring-3 focus-visible:ring-primary-500 focus-visible:ring-offset-2"
                    aria-haspopup="listbox" aria-expanded="false" :aria-expanded="open">
                    <span class="truncate">
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-100"
                    ...
                    <div class="px-3 py-2 border-b border-gray-200 dark:border-gray-600 flex gap-2">
                        ...
                        <button type="button" wire:click="deselectAllStatuses"
                    ...
                    @foreach ($this->availableStatuses as $statusValue => $statusLabel)
                        <label wire:key="status-{{ $statusValue }}"
                            ...
                            <input type="checkbox" wire:click="toggleStatus('{{ $statusValue }}')"
        ...
        <div class="space-y-2">
            ...
            <select id="filter-category" wire:model.live="selectedCategory"
                ...
                @foreach ($this->availableCategories as $category)
                    <option wire:key="category-{{ $category->id }}" value="{{ $category->id }}">{{ $category->name }}
                    </option>
                @endforeach
            </select>
        </div>
        ...
        @if ($filterType === 'helpdesk')
            <div class="space-y-2">
                ...
                <select id="filter-priority" wire:model.live="selectedPriority"
                    class="w-full px-4 py-2 min-h-11 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm text-sm text-gray-900 dark:text-white focus:outline-none focus-visible:ring-3 focus-visible:ring-primary-500 focus:border-primary-500"
                    ...
                    @foreach ($this->availablePriorities as $priorityValue => $priorityLabel)
                        <option wire:key="priority-{{ $priorityValue }}" value="{{ $priorityValue }}">
        ...
        <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
            <div class="flex flex-wrap gap-2">
                ...
                @foreach ($selectedStatuses as $status)
                    <span wire:key="active-status-{{ $status }}"
                        ...
                        <button type="button" wire:click="toggleStatus('{{ $status }}')"
                ...
                @if ($dateFrom || $dateTo)
                    <span
                        class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-info-100 dark:bg-info-900 text-info-800 dark:text-info-200">
                        {{ __('portal.date_range') }}:
                        {{ $dateFrom ?? __('portal.any') }} - {{ $dateTo ?? __('portal.any') }}
                        <button type="button" wire:click="$set('dateFrom', null); $set('dateTo', null)"
                ...
                @if ($selectedCategory)
                    ...
                    @endphp
                    <span
                        class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-secondary-100 dark:bg-secondary-900 text-secondary-800 dark:text-secondary-200">
                        {{ $filterType === 'helpdesk' ? __('portal.category') : __('portal.asset_type') }}:
                        {{ $categoryName }}
                ...
                @if ($filterType === 'helpdesk' && $selectedPriority)
                    <span
                        class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-warning-100 dark:bg-warning-900 text-warning-800 dark:text-warning-200">
                        {{ __('portal.priority') }}: {{ $this->availablePriorities[$selectedPriority] }}
                        <button type="button" wire:click="$set('selectedPriority', null)"
    ...
    <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
        <button type="button" wire:click="applyFilters"
            class="w-full inline-flex items-center justify-center px-6 py-3 min-h-11 bg-primary-600 hover:bg-primary-700 text-white font-medium rounded-lg shadow-sm focus:outline-none focus-visible:ring-3 focus-visible:ring-primary-500 focus-visible:ring-offset-2 transition-colors duration-200"
    ...
    {{-- Loading Overlay --}}
    <div wire:loading wire:target="applyFilters,clearFilters,toggleStatus,selectAllStatuses,deselectAllStatuses"
        class="absolute inset-0 bg-white/75 dark:bg-gray-800/75 flex items-center justify-center rounded-lg">
