...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-8" x-data="optimisticHelpdeskForm()"
    ...
    {{-- ARIA Live Region for Screen Reader Announcements (must be inside single root) --}}
    ...
    <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        @else
                ...
                <nav aria-label="Progress">
                    <ol class="flex items-center justify-between">
                        @foreach (range(1, $totalSteps) as $step)
                            <li class="flex-1 {{ $step < $totalSteps ? 'pr-2' : '' }}">
            ...
            <form wire:submit="submit" class="space-y-6" aria-labelledby="form-heading">
                    ...
                    <section class="{{ $sectionCardClasses }} space-y-6" aria-labelledby="personal-info-heading">
                        <div
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                {{ __('Enter your contact details.') }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <x-form.input wire:model.live.debounce.300ms="job_grade" label="{{ __('Job Grade') }}"
                                required />
                        </div>

                        {{-- Searchable Division Select (Virtual Scrolled Combobox) --}}
                        {{-- @trace Task 3.1.10 - Implement Searchable Division Select --}}
                        <x-form.searchable-select name="division_id" label="{{ __('Division') }}" :options="$this->divisions->map(fn($d) => ['id' => $d->id, 'name' => $d->name])->toArray()"
                            :selected="$division_id" placeholder="{{ __('Select a division') }}"
                ...
                @if ($currentStep === 2)
                    <section class="{{ $sectionCardClasses }} space-y-6" aria-labelledby="issue-details-heading">
                        <div
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                {{ __('Describe your technical issue.') }}</p>
                        </div>
                ...
                @if ($currentStep === 3)
                    <section class="{{ $sectionCardClasses }} space-y-6" aria-labelledby="declaration-heading">
                        <div
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                {{ __('Review and confirm your submission.') }}</p>
                        </div>
...
{{-- Alpine.js components are now globally registered in resources/js/alpine-components.js --}}
