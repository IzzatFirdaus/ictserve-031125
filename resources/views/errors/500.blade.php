    ...
    <div class="flex min-h-screen items-center justify-center bg-gray-50 px-4 py-12 sm:px-6 lg:px-8">
        <div class="w-full max-w-md text-center">
            ...
            <div class="mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-danger-100">
                <x-heroicon-o-exclamation-triangle class="h-12 w-12 text-danger-600" />
            </div>
            ...
            <p class="mt-4 text-base text-gray-600">
                {{ __('portal.errors.server_error') ?? 'Something went wrong on our end. We\'re working to fix it.' }}
            </p>

            {{-- Suggestion --}}
            <p class="mt-2 text-sm text-gray-500">
                {{ __('portal.errors.try_again_later') ?? 'Please try again in a few moments.' }}
            </p>
            ...
            <div class="mt-8 rounded-lg border border-gray-200 bg-white p-4 text-left">
                <h3 class="text-sm font-medium text-gray-900">
                    {{ __('portal.errors.persistent_issue') ?? 'Issue persists?' }}
                </h3>
