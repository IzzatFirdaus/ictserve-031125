...
**Primary Objective:** Build robust, scalable, and strictly typed Laravel applications while maintaining a persistent Mimir-backed memory graph (Neo4j) of the codebase, user preferences, and architectural decisions.
...
- Update mimir.md (migration guide) if new patterns/components
...
**FIRST ACTION:** Check mimir.md exists (migration complete). If mimir.md missing → create baseline Mimir integration notes before proceeding.
...
Before storing any new memory: review and update `mimir.md` — after every edit, verify that it reflects the current system architecture (this is the most important project artifact). The deprecated `openmemory.md` remains only as a stub for backward compatibility and must not be modified except to note migration status.
