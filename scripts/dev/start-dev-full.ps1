    ...
    for ($i = 0; $i -lt $Attempts; $i++) {
        try {
            ...
            if ($result.TcpTestSucceeded) {
                ...
                if ($HealthEndpoint) {
                    try {
                        $healthUrl = "http://127.0.0.1:$Port$HealthEndpoint"
...
function Start-WSLRedis {
    ...
    $redisCmd = 'redis-server --daemonize yes --port 6379 --bind 127.0.0.1 2>/dev/null && echo started || echo failed'
    $startResult = & wsl.exe -e bash -c $redisCmd 2>$null
    ...
    $sudoCmd = 'timeout 2 sudo -n redis-server --daemonize yes --port 6379 --bind 127.0.0.1 2>/dev/null && echo started || echo failed'
    $sudoResult = & wsl.exe -e bash -c $sudoCmd 2>$null
...
if ($servicesToStart -contains "redis") {
    ...
    if ($wslRedis.Available -and $wslRedis.HasRedis) {
        ...
        $redisMonitorCmd = "wsl.exe redis-cli ping; Write-Host 'Redis ready for Laravel!' -ForegroundColor Green; Write-Host 'Monitoring Redis commands...' -ForegroundColor Yellow; wsl.exe redis-cli monitor"
        Start-DevService -Title "Redis Server (WSL Monitor)" -Command $redisMonitorCmd -Color "Red" -Description "Cache, Sessions, Queues, Reverb Backend" -Priority 1

        # Verify Redis is accessible
        Test-ServicePort -Port 6379 -Attempts 10 -DelaySeconds 1 -ServiceName 'Redis (WSL)'
        ...
        if ($InstallRedis) {
            ...
            $installCmd = 'sudo apt update && sudo apt install -y redis-server && echo installed || echo failed'
            $installResult = & wsl.exe -e bash -c $installCmd 2>$null
        ...
        $redisFound = Test-ServicePort -Port 6379 -Attempts 3 -DelaySeconds 1 -ServiceName 'Alternative Redis'
        if (-not $redisFound) {
        ...
        $redisFound = Test-ServicePort -Port 6379 -Attempts 5 -DelaySeconds 1 -ServiceName 'Local Redis'
        if (-not $redisFound) {
            ...
            Write-Host "      - Laragon: Enable Redis module" -ForegroundColor Gray
...
if ($servicesToStart -contains "laravel") {
    ...
    Start-DevService -Title "Laravel Server" -Command "php artisan serve --host=127.0.0.1 --port=8000" -Color "Blue" -Description "ICTServe v3.6.0 - True Hybrid Architecture" -Critical -Priority 2
    Test-ServicePort -Port 8000 -Attempts 15 -DelaySeconds 1 -ServiceName 'Laravel Server' -HealthEndpoint "/" -Critical
    Start-Sleep -Seconds 1
}
...
if ($servicesToStart -contains "reverb") {
    ...
    Start-DevService -Title "Laravel Reverb" -Command "php artisan reverb:start" -Color "Magenta" -Description "Real-time notifications, live updates, broadcasting" -Priority 3
    Test-ServicePort -Port 8080 -Attempts 15 -DelaySeconds 1 -ServiceName 'Laravel Reverb'
    Start-Sleep -Seconds 1
}
...
if ($servicesToStart -contains "queue") {
    ...
    $queueCmd = "php artisan queue:work --tries=3 --timeout=90 --sleep=3 --max-jobs=1000 --max-time=3600"
    Start-DevService -Title "Laravel Queue Worker" -Command $queueCmd -Color "Cyan" -Description "Email notifications, file processing, audit logging" -Priority 4
...
if ($servicesToStart -contains "mcp" -and -not $NoMCP) {
    ...
    Start-DevService -Title "Laravel MCP Server" -Command "php artisan boost:mcp" -Color "DarkCyan" -Description "Model Context Protocol for AI chatbot integration" -Priority 6
...
if ($servicesToStart -contains "pulse") {
    ...
    Start-DevService -Title "Laravel Pulse Monitor" -Command "php artisan pulse:check" -Color "DarkGreen" -Description "Performance metrics, slow queries, exceptions" -Priority 7
    Start-Sleep -Seconds 1
}
...
if ($servicesToStart -contains "queue") {
    Write-Host "  [QUEUE] Queue Worker          - Background Jobs and Email Processing" -ForegroundColor Cyan
}
if ($servicesToStart -contains "vite") {
    Write-Host "  [VITE] Vite Dev Server        - Frontend Assets + HMR (127.0.0.1:5173)" -ForegroundColor Green
}
if ($servicesToStart -contains "mcp" -and -not $NoMCP) {
    Write-Host "  [MCP] Laravel MCP Server      - AI Integration and Chatbot" -ForegroundColor DarkCyan
}
if ($servicesToStart -contains "pulse") {
...
Write-Host "  [PDPA] PDPA 2010: Personal data encryption and audit logging active" -ForegroundColor Gray
