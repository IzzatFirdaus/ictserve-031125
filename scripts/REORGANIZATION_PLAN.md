...
## Overview
This document outlines the reorganization of scripts in the `scripts/` directory into appropriate subdirectories while preserving scripts used by `npm run dev:win` and `start-dev.ps1`.

## Scripts to Preserve (DO NOT MOVE)
...
- `fix-npm.ps1` - NPM configuration fixes
...
1. Create target subdirectories if they don't exist
2. Move scripts to appropriate locations
3. Update any internal references between scripts
4. Test that npm run dev:win and start-dev.ps1 still work
