...
- copies .env.docker -> .env when .env does not already exist
- runs docker compose up -d --build and reports status

Note: This is written for Windows PowerShell / PowerShell Core.
#>
...
if (-not (Test-Path -Path .env)) {
    if (Test-Path -Path .env.docker) {
        Write-Host ".env not found - copying .env.docker to .env" -ForegroundColor Yellow
        Copy-Item -Path .env.docker -Destination .env -Force
    } else {
        Write-Host ".env.docker missing - please create or copy the correct environment file." -ForegroundColor Red
        exit 1
    }
} else {
...
Write-Host "To run migrations and seeders execute: docker compose exec app php artisan migrate --seed" -ForegroundColor Green
