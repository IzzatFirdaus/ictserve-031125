...
| ----- | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| 3.6.1 | 17 Disember 2025 | Kemaskini teknologi stack: Laravel 12.43.1, Livewire 3.7.3, Playwright 1.57.0, Percy 1.31.6, Tailwind CSS 4.1.18. Tambah §14.3 Percy Visual Testing, §14.4 BrowserStack Integration, §14.5 MCP Testing Integration. Dokumentasi lengkap untuk Percy + Playwright + BrowserStack MCP workflows. Laravel Dusk deprecated. Penyelarasan dengan D00-D18 v3.6.1. | Pasukan BPM |
...
| **Percy**        | 1.31.6  | Visual regression testing         |
| **BrowserStack** | Latest  | Cross-browser testing (optional)  |
...
### 14.3. Percy Visual Testing

Percy adalah platform visual regression testing yang digunakan untuk mengesan perubahan visual merentasi pelbagai browser dan viewport. ICTServe v3.6.1 menggunakan Percy sebagai pengganti kepada Laravel Dusk yang sudah usang.

**Teknologi Stack:**
- **Percy CLI**: v1.31.6
- **Percy Playwright**: v1.0.10
- **Playwright**: v1.57.0

**Konfigurasi Percy** (`percy.config.js`):
...
export default {
  ...
  snapshot: {
    // Responsive breakpoints untuk ICTServe
    widths: [375, 768, 1024, 1280, 1920],
    minHeight: 1024,
    
    // CSS untuk menyembunyikan kandungan dinamik
    percyCSS: `
...
```bash
# Jalankan ujian dengan Percy
npm run test:e2e:percy

# Ujian Percy untuk modul tertentu
npm run test:e2e:percy:helpdesk
npm run test:e2e:percy:loan

# Ujian aksesibiliti dengan Percy
...
**Ciri-ciri Percy untuk ICTServe:**
- **True Hybrid Architecture Support**: Ujian untuk tetamu, pengguna disahkan, dan admin
- **Bahasa Melayu Validation**: Pengesahan antara muka Bahasa Melayu
- **Livewire Component Testing**: Sokongan untuk komponen server-driven UI
- **Filament Admin Testing**: Ujian visual untuk panel admin
- **WCAG 2.2 AA Compliance**: Pengesahan aksesibiliti
- **Graceful Degradation**: Ujian berjalan tanpa Percy jika perkhidmatan tidak tersedia

**Rujukan**: `docs/Percy-Playwright-Integration.md`

### 14.4. BrowserStack Integration

BrowserStack menyediakan platform ujian cross-browser dan peranti sebenar untuk memastikan ICTServe berfungsi dengan baik merentasi pelbagai pelayar dan peranti.

**Konfigurasi BrowserStack** (`config/browserstack.php`):
...
return [
    ...
    'accessibility_testing' => [
        ...
        'wcag_level' => 'AA', // WCAG 2.2 AA untuk ICTServe
...
ICTServe v3.6.1 menyediakan BrowserStack MCP (Model Context Protocol) server untuk integrasi AI-assisted testing:

```bash
# Mulakan BrowserStack MCP server
php artisan mcp:browserstack
```

**Tools yang tersedia:**
- `browserstack_validate_config` - Pengesahan konfigurasi
- `browserstack_get_browsers` - Dapatkan pelayar dan peranti yang tersedia
- `browserstack_create_percy_session` - Cipta sesi Percy visual testing
- `browserstack_run_percy_visual_test` - Jalankan ujian visual Percy
- `browserstack_accessibility_test` - Ujian aksesibiliti dengan Percy
- `browserstack_wcag_compliance_scan` - Imbasan pematuhan WCAG 2.2 AA
- `browserstack_create_live_session` - Cipta sesi Live untuk debugging
- `browserstack_run_cross_browser_test` - Ujian cross-browser visual

**Pelayar dan Peranti Disokong:**

| Platform | Pelayar/Peranti | Versi |
|----------|----------------|-------|
...
| **Tablet** | iPad Pro 12.9 | iOS 16 |
| | Samsung Galaxy Tab S9 | Android 13.0 |

**Penggunaan BrowserStack:**

```bash
# Ujian cross-browser dengan Percy
npm run test:percy:browserstack

# Ujian aksesibiliti BrowserStack
npm run test:browserstack:accessibility

# Ujian mobile visual
npm run test:browserstack:mobile
```

**Integrasi dengan Percy:**

BrowserStack dan Percy bekerjasama untuk menyediakan ujian visual regression merentasi pelbagai pelayar:

```typescript
test('Cross-browser visual test', async ({ page }) => {
  await page.goto('/');
  
  // Tangkap snapshot merentasi pelbagai pelayar via BrowserStack
  await percySnapshot(page, 'Landing Page - Cross-Browser', {
    widths: [375, 768, 1280, 1920],
  });
});
```

**Live Session untuk Debugging:**

BrowserStack Live membolehkan debugging interaktif dengan integrasi Percy:

- Manual testing pada peranti sebenar
- Collaborative debugging dengan pasukan
- Percy snapshot capture semasa sesi Live
- Visual issue tracking dan reporting

**Rujukan**:
- `docs/Percy-Playwright-Integration.md` - Panduan integrasi lengkap
- `config/browserstack.php` - Konfigurasi BrowserStack
- `.mcp.json` - Konfigurasi BrowserStack MCP server
- `app/Mcp/BrowserStackMcpServer.php` - Implementasi MCP server

### 14.5. MCP (Model Context Protocol) Testing Integration

ICTServe v3.6.1 menyediakan integrasi MCP untuk AI-assisted testing workflows:

**MCP Servers untuk Testing** (`.mcp.json`):
...
```json
{
  "mcpServers": {
    ...
    "percy": {
      "command": "node",
      "args": ["scripts/mcp/percy-mcp-server.cjs"],
      "disabled": true,
      "autoApprove": [
        "percy_snapshot",
...
**Workflow MCP Testing:**

1. **Playwright MCP** - Browser automation dan E2E testing
2. **Percy MCP** - Visual regression testing
3. **BrowserStack MCP** - Cross-browser dan cross-device testing

**Rujukan**: `tests/e2e/mcp-percy-browserstack-integration.spec.ts`
